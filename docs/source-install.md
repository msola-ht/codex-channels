...
只有这些步骤全部通过，才停止已安装的核心服务，安装配套 CLI、切换源码并刷新全局命令，调用目标版本的数据库升级入口，成功后恢复服务并确认就绪。更新不修改用户配置或 Provider 模型目录，也不处理历史服务和 PATH。

本次清理以现有数据库结构为基线，移除基线之前的迁移，不取消后续数据库升级。当前入口只复核版本与结构，不写库。后续 Schema 变更由目标版本的只读预检声明是否需要升级，停服后的执行入口负责备份、迁移及目标结构校验；不支持的起始版本在预检时拒绝。运行时不隐式迁移，更新不会用清空数据库代替升级。
...
仓库保存在错误消息给出的 `codex-channels.pre-update-*` 路径。数据库升级已完成或无需升级时，命令会尝试恢复已停止的核心服务，恢复也失败时同时报告更新与服务错误。切换后数据库升级尚未完成时保持服务停止，检查错误及迁移备份后再重试；不能只回退源码就启动服务。

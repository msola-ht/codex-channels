...
只有这些步骤全部通过，才停止核心服务并切换源码；新源码随后复用统一配置与数据库更新流程
（包括会话展示缓存的兼容检查与必要重建）并恢复服务。仍包含历史远程指标中心的安装会自动停止并
注销旧本机中心服务，备份配置后移除 `[metrics.sync]`、`[metrics.center]` 和 `[metrics.view]`；
旧中心数据库、同步水位和外部 Cloudflare 资源继续保留。同一版本号下的修复提交也会更新。npm
安装模式不更新 Gateway npm 包，但会先同步其锁定的配套 Codex CLI，再更新本地配置和数据库。

从任意开发仓库运行 `npm run install:global` 会构建并安装当前源码的 Gateway 包，配套 CLI
由随后执行的 `codexc update` 同步；这个流程不把开发仓库登记为受管 `main` 仓库，也不拉取或
切换开发分支。CLI 版本不一致时，同样先询问、临时安装和校验，通过后才在已安装服务的停机窗口内
同步全局 CLI，并继续本地更新。临时候选目录会在成功或失败后清理。
受管源码已经是远端最新提交时也复用这套流程，仍会检查并同步配套 CLI，然后执行一次本地更新，
不重新克隆或构建源码。

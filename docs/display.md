...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  ...
  从本轮开始到完成的总耗时。OpenAI 提供有效指标时，性能区另显示“首字耗时”，取当前 Thread/Turn
  ...
  模型请求聚合耗时、本地首段回复延迟和生成速度不再作为统计项展示，代理
  也不再为这些输出追踪每个流式增量的时间。当前锁定 Codex 0.154.0 的首轮 `generate=false` WebSocket 预热不属于模型推理，

...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  中断、未完整观测和失败数量，同时展示 Token 与缓存命中。总耗时、首段回复延迟和生成速度不再
  作为统计项展示，代理也不再为这些输出追踪每个流式增量的时间。当前锁定 Codex 0.153.4 的首轮 `generate=false` WebSocket 预热不属于模型推理，
...
`/usage` 的账户来源，不会为其查询当前 OpenAI Thread。官方估算按当前锁定的 0.153.4 App Server 活动
...
当前锁定的 Codex CLI 0.153.4 没有独立压缩模型配置；常规压缩复用 Turn 模型，模型切换兼容压缩
可能先使用上一模型，并在受支持的失败条件下回退当前模型。

完成卡片的模型/提供商之下会单列「自动压缩：X%」。受管 Provider 读取模型目录的压缩阈值；
官方模型优先读取主配置覆盖值（`model_context_window` / `model_auto_compact_token_limit`），
未覆盖时显示默认 95%。

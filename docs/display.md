...
- 会话区在正式与调试模式都显示当前工作区和 Codex Thread 会话 ID，保留 Thread 累计上下文、压缩次数、Goal 与 Git 分支，并显示递归包含全部已关联子代理后代的
  模型请求数、Token 总量、总价和全量计价时的均价；历史上只有 `parent_turn_id = NULL` 的子代理仍计入
  会话累计，但不会被猜测归属到任何父 Turn。
- 父 Turn 确有直接子代理时，运行区另显示“任务合计（含子代理）”，按父 Turn 自身及其全部后代的显式
  `parent_turn_id` 归属汇总请求数、Token、费用和均价；该块不混入子代理速度，父 Turn 的“本次运行”模型

...
- 会话区在正式与调试模式都显示当前工作区、Session 名称和 Session ID，保留 Session 累计上下文、压缩次数、Goal 与 Git 分支，并显示递归包含全部已关联子代理后代的
  模型请求数、Token 总量、缓存命中率、总价和全量计价时的均价；缓存命中率与 Token 总计一样默认
...
- 完成卡片正式模式在本次运行和当前 Session 累计之外，保留精简的“账户状态”：OpenAI 优先显示
  指标中心多设备汇总的周限剩余百分比，中心不可用时显示官方周限；OpenCode Go 等 Provider 在
  中心存在对应额度周期时显示这些周期的精简摘要，5 小时窗口尚未同步时使用最近官方账户快照补齐，
  中心不可用时显示本机剩余用量；
  启动卡片按 5小时、7天、30天顺序分块展示三个窗口的设备数、请求数和 Token 摘要，调试模式才在完成卡片
  进一步展开额度中心细节；官方用量接口或本地指标不可用时
  对应字段不显示。完整额度详情仍通过 `/limits` 或 `/usage` 查看。

...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  ...
  从本轮开始到完成的总耗时。OpenAI 提供有效指标时，性能区另显示“上游轮次首 Token”，取当前 Thread/Turn
  首个有效普通响应的上游 `first_sampled_message_ttft_ms`（`logical_turn` 范围）样本，不累加或平均重复回报，
  不包含压缩与子代理，不等于客户端首字延迟；缺失时不显示、不估算，零值保留。该数值随请求写入
  指标库，Gateway 重启后可恢复；子代理完成卡片只读取已明确终态 Turn 自身的样本，不从历史轮次猜测。
  `/metrics` 最近运行聚合同样标明该轮次口径；单请求首内容延迟仅在请求明细、导出和转储中展示，不合成为轮次首内容。
  模型请求聚合耗时和生成速度不作为统计项展示，代理只记录每个请求首个有效内容增量的耗时。

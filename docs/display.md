...
三个渠道的输出路由均按完整会话身份隔离，同一会话保持顺序。完成卡片的统计补全最多等待 5 秒，
超时保留已经取得的本轮统计，无法可靠取得的累计值省略，不阻塞其他会话和渠道。普通输出在 Gateway
关闭时最多排空 5 秒，交互立即取消；到期后停止后续分片和回退发送。

Telegram 的 `message_format = "html"` 会把已完成、非 commentary 的短回复转换为 HTML，
即使上游未携带 phase 也会转换；流式增量仍使用纯文本，完成后原地更新格式。
超过 3,500 字符的普通回复分段渲染 HTML，标题、粗体、链接和代码样式随分片保留。
超过 16,000 字符或包含至少 80 行代码的长回复发送预览与 Markdown 文件；文件保留完整原文。
分片格式被明确拒绝时仅回退该片段，不重发已经成功发送的片段。
明确的格式拒绝仍允许回退纯文本，网络故障不触发格式回退。

Telegram 的最终回复等关键 API 调用在临时网络故障或限流后有限重试：单次最多等待 30 秒，
每次调用总恢复预算为 120 秒，最多尝试 6 次。发送新消息时，如果无法确认前一次是否已经送达，
停止自动重试，避免重复创建。明确的连接建立失败仍可重试。思考、过程增量等非关键更新最多等待 5 秒且不重试。
同一聊天的后续输出会等待当前调用完成，不同聊天独立发送；这些等待不会暂停模型生成。
网络故障不触发纯文本重发。重试耗尽会记录最终失败与安全的网络错误码；Gateway 输出只保留在内存中，
不保证重启后补送旧回复。

三渠道每项输出从出队开始共用 120 秒取消预算，长回复的分片、卡片收尾和回退不重新计时；
该预算不包括排队时间，日志分别记录排队和执行耗时。取消向平台请求传递，但无法撤销平台已经接受的消息。
飞书卡片和微信文件投递结果未知时不再自动创建替代消息；只在明确可安全回退的失败下换用其他格式。
Telegram 命令回复与普通输出共用会话发送队列；不同聊天的普通入站处理互不等待。
微信同一已拉取批次内的 `/stop` 可以越过慢任务，批次游标仍等待普通输入处理完成后提交；尚未拉取的停止指令仍需等待下一次轮询。
...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  ...
  完成卡片只给出一处速度读数“输出 Token/s”（未采样时整行省略），`/metrics` 的最近运行聚合与当前会话累计使用同一读数与同一公式：
  `outputTokens × 1000 / totalDurationMs`。分子是全部输出 Token、不做任何扣除，含推理 Token 与工具调用参数；分母是由采集端单调时钟
  从代理提交上游请求计到首次模型终态的总耗时，含上游排队与思考产生的首字等待，无终态时到结束或失败。因此该读数覆盖请求发出到结束，
  包含首字耗时，不等于首字之后的纯解码速度。Turn/Thread 与范围汇总都按「合计输出 Token ÷ 合计请求耗时」合并计算，不先把逐请求速率平均，
  只在输出 Token 与总耗时同时大于零的记录上取样，缺采样的记录整条退出：失败请求通常没有用量样本，因此不参与，压缩请求沿用原统计范围。
  新请求的首内容耗时与请求总耗时统一从代理提交上游请求开始计时，排除发送前准备及 WS 连接等待；历史采样保持原值，派生速率按当前公式
  连同历史记录一起重算。历史缺少时长或无有效样本时整行省略（`/metrics` 文本显示“未提供”），不按旧时间戳补算。正式和调试模式均显示；
  该派生值不新增存储字段，也不是会话墙钟吞吐量：它不含请求之间的编排、工具执行与空闲时间。
  请求明细和转储的“首字耗时”从上游转发开始计到首个内容帧，HTTP 与 WebSocket 共用同一份口径：
  只认 `response.*` 中以 `.delta` 或 `.done` 结尾的内容事件，不计生命周期、条目与分片边界、终态以及旁路元数据，
  不要求事件已经携带可见文本；详见[WebUI 请求明细](webui.md)。
...
OCG 与 Cline Pass 的 `/usage` 同样按各账户的 5 小时、7 天和月度窗口显示本机 Token/百分点估算、满额外推及有效区间覆盖；未形成区间显示“正在采样”，读取失败显示“暂不可用”。使用后再次查询额度可形成新快照。数据由既有账户快照与本地请求只读计算，估算不写入原始快照，也不作为官方兑换率或余额。完整采样与失效口径见 [WebUI 额度说明](webui.md)。
...
在当前持久交接路径中，Telegram、飞书的可见 MCP、动态工具和其他操作完成通知逐项发送并确认，
不把仅保存在内存汇总器的结果算作已送达。独立使用、未启用可靠交接的 Outbox 仍可延迟汇总查询结果。
显示级别继续控制详情与隐藏行为，微信继续保留原有回复窗口预算。

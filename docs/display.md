...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  ...
  `generate=false` WebSocket 预热不属于模型推理，不计入请求数、Token 或耗时。
...
- 父 Turn 确有直接子代理运行时，运行区另显示“任务合计（含子代理）”，按 Schema v11
  ...
  子项；v10 以前的历史运行关系不按时间补推。该块不混入子代理速度，父 Turn 的“本次运行”模型
...
- Gateway 不在本地计算或估算模型价格与费用，完成卡片不展示总价、均价、单价或峰谷档位，也不
  保存价格快照或刷新价格目录。账户与额度仍展示官方来源的数据：OpenAI 周限与额度中心摘要、
...
不把未知值冒充零。卡片展示任务名、模型、提供商、可可靠观测的思考等级、从启动到首次官方终态的墙钟耗时、请求次数与 Token；
墙钟耗时不包含终态后等待指标落库的收敛时间，并在指标不可用时继续显示；
线程聚合存在有效输出速度样本时，显示“综合输出速度：…（不含推理 · 覆盖 x/y 次请求）”；没有正数速度或有效覆盖计数时不展示速度。
正式模式保留 Token 总计；调试模式才展开输入缓存、未缓存输入、输出、推理输出、缓存命中率，
并显示模型请求聚合耗时。
...
启动通知的“运行环境”小节、完成通知的 Token 子项、“模型请求聚合耗时”和

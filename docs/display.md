...
- 会话区在正式与调试模式都显示当前工作区和 Session ID，保留 Thread 累计上下文、压缩次数、Goal 与 Git 分支，并显示递归包含全部已关联子代理后代的
...
- OpenAI ChatGPT 订阅请求的参考费用不代表实际账单或额外扣费。DeepSeek 使用随版本审查的官方
  ...
  缓存。所有刷新都不会阻塞模型请求。
- 完成卡片正式模式只展示本次运行和当前 Session 累计；额度中心、OpenAI 周限和 Provider 剩余用量
  不在每次完成通知中重复展示。调试模式才在“账户状态”区展示额度中心周期汇总、OpenAI 周限和
  Provider 剩余用量；官方用量接口或本地指标不可用时对应字段不显示。额度详情统一通过
  `/limits` 或 `/usage` 查看。

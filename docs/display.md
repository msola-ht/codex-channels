...
在 `codexc config` 中选择“系统设置 → 调试模式（快捷开关）”可全局开启或关闭脱敏调试信息。开启后会把
`[logging].level` 设为 `debug`，同时启用 Gateway 各模块的脱敏调试日志，并在渠道中显示
启动通知的“运行环境”小节和完成通知的 Token 子项；关闭后恢复为 `info`，隐藏这些调试字段，但保留完成通知的
Token 总计。
修改后只需重启 Gateway：
...
需要查看模型请求与响应的完整字段时，在 `codexc config` 中选择“系统设置 → 模型请求转储”，

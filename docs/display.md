...
- 运行区把统计代理捕获到的本 Turn 全部模型请求聚合为请求次数，并在存在异常尝试时拆分完成、
  ...
  综合输出速度，并显示具有完整 Token 与流式时间窗的请求覆盖率。Codex 0.148.0 首轮
...
- 会话区在正式与调试模式都显示当前工作区和 Codex Thread ID，保留 Thread 累计上下文、压缩次数、Goal 与 Git 分支，并显示递归包含全部已关联子代理后代的
...
- OpenAI ChatGPT 订阅请求的参考费用不代表实际账单或额外扣费。DeepSeek 使用随版本审查的官方
  ...
  缓存。所有刷新都不会阻塞模型请求。
- 完成卡片在“账户状态”区展示当前模型的剩余用量（当前由 OpenCode Go 注入，其余 Provider 无用量
  查询时不显示）：按当前官方价格基线重算的剩余金额、月度包含额度与已用百分比（与 `/usage`
  同一口径）；DeepSeek 模型按该 Turn 最后一次模型请求开始时间所在时段的 Off-Peak / Peak 档位
  展示对应剩余额度（如“剩余用量（Peak）”），没有请求时间记录时回退当前时间；官方用量接口或
  本地指标不可用时该字段不显示。
...
`/usage` 的账户来源，不会为其查询当前 OpenAI Thread。官方估算按 0.148.0 App Server 当前活动
...
当前锁定的 Codex CLI 0.148.0 没有独立压缩模型配置；常规压缩复用 Turn 模型，模型切换兼容压缩
...
`interacted` 与 `interrupted` 不会被误显示为新建子代理。子线程 `turn/started` 即使先于父线程
`interacted` 到达，也会与待处理活动合并为同一轮；延迟到达的上一轮终态或指标不会污染新一轮。官方终态后

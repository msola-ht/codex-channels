...
`【序号】`；Run 的计划时间、Thread 和错误分类作为该 Run 的二级项，避免渠道 Markdown 重置序号或抹平层级。当前四种计划都是循环定义，完成的 Run
进入 `/schedule runs` 历史，任务继续保留并计算下次运行，不自动删除。`/schedule <自然语言>` 会启动一个不持久化、不绑定会话且受 JSON Schema
约束的临时 Thread；Gateway 只消费其最终结构化草案，完成后取消订阅，随后仍
使用同一创建预览与确认链路。缺少明确 IANA 时区或计划类型不受支持时只返回可操作提示，不创建任务。

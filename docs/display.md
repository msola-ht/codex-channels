...
- OpenAI ChatGPT 订阅请求的参考费用不代表实际账单或额外扣费。DeepSeek 使用随版本审查的官方
  ...
  缓存。所有刷新都不会阻塞模型请求。
- OpenCode Go Thread 的完成卡片在“账户状态”区展示当前模型的剩余用量：按当前官方价格基线
  重算的剩余金额、月度包含额度与已用百分比（与 `/usage` 同一口径）；DeepSeek 模型按该 Turn
  最后一次模型请求开始时间所在时段的 Off-Peak / Peak 档位展示对应剩余额度（如“剩余用量
  （Peak）”），没有请求时间记录时回退当前时间；官方用量接口或本地指标不可用时该字段不显示。

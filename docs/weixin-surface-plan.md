...
- 进行中：微信 `/status` 已追加当前进程内的轮询、短重试、退避、Token 失效暂停或停止状态，
  ...
  时间；不显示 Token、`context_token`、游标或原始响应，也不竞争消费 `getupdates`。账号、最近
  授权上下文、游标检查点和凭据可用性的独立 Doctor 观测仍待后续切片。
...
- 只接受完整 `/approve <id> once|session|rule|network <序号>` 或 `/deny <id>`；不得接受
  “回复 1”“回复同意”、别名或宽松文本解析。
...
2. 一般文件：隔离的入站合同探针已按固定 `v2.4.6` 实现完成态文件筛选、官方 CDN 下载、
   ...
   正文以内联用户文本进入 App Server Thread；
   当前锁定 Codex 协议没有通用文件输入，因此二进制文件明确拒绝，文件上传失败语义仍须另立切片。

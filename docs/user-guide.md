...
照常在渠道发送图片即可，不需要额外命令。Gateway 校验图片后以内联输入提交，
由官方 App Server 完成模型相关的图片准备并发送给模型。现有 Actor、Workspace、模型能力和媒体大小限制继续适用。

当前没有接入自动上传并转换为 `fileId`，也没有文件管理或下载入口；
“避免后续请求重复携带图片 Base64”仍是未交付能力。
协议已支持已有图片引用，探测和合同已验证引用转发与历史保留，但这不等于渠道自动引用已可用。
方案状态见[图片引用决策](codex-cli-upgrade-decisions.md#图片文件引用需求阻塞与实现边界)。

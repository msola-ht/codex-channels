...
与响应模型、状态；选中某条后才会读取并显示请求头、请求体、响应头、SSE 事件或 WebSocket 双向帧。
单段正文超过 4 MiB 时只返回前 4 MiB，并在响应里带 `bodyTruncated`。`label` 只接受 `traffic/` 中
已存在的标签，非法或无来源的取值分别返回 400 与 404；目录中还没有转储文件、或请求不来自回环地址时
返回 503，响应里的 `enabled` 表示 `[debug].model_traffic_dump` 当前是否开启。

...
返回列表保留原筛选与页码，刷新可回到同一条记录。摘要表的一行就是一次模型调用，不展示批次内编号；点击时间或记录行打开明细。列表显示时间、路径或 WebSocket URL、线程、
轮次、提供商、请求类型、请求/响应模型和终态，并区分模型列表查询、连接预热与模型请求。
列表 API 未传 `label` 时返回全部提供商，响应顶层 `label` 为 `null`，每条摘要的 `label` 保留实际提供商；分页和每页条数调整不自动收窄筛选。
详情标题统一为“调用明细”，顶部集中展示提供商、完整批次和调用编号；编号为批次内计数，新批次重新开始，不是全局请求序号。“复制定位信息”复制这三个定位字段，成功或失败均显示提示。
明细页面包屑显示“调用详情 → 调用明细”，返回列表保留筛选与分页。首字、总耗时、用量和失败原因前置；详细耗时、模型声明与参数对照在诊断区默认收起。原始正文、输入条目、工具定义及事件正文展开后才挂载和格式化，后端仍读取轨迹以提取输出与模型证据。正文支持复制原文、自动换行及 JSON 格式化切换；已截断内容只复制保留片段，不提供格式化。事件区显示当前记录范围、加载反馈与就近重试，刷新期间明确说明摘要来自上次成功读取。
点开后固定显示一张请求卡和一张响应卡：请求卡展示思考等级、请求服务层级和接续响应 ID；响应卡
展示实际服务层级，以及按顺序排列的回答、推理摘要与工具调用；输入/缓存/输出/推理 Token 统一在顶部摘要展示。
...
诊断区和 CLI 同时列出响应头及元数据中 `X-Codex-Turn-State` 字段值的字符数与来源；按解析后的 Unicode 码点计数，不包含头名称或 JSON 转义符，不解码内容。相同来源与长度去重，缺失显示“未记录”，空字符串为 0；新增统计只返回长度，不复制字段原文。

调用列表在总耗时后显示“Turn State 字符数”：不同长度用 `/` 分隔，不累加，读取成功但缺失显示 `—`，提示中列出来源。列表先返回摘要，字符数通过 `GET /api/v1/traffic/turn-state?label=&session=&ids=` 按当前页的精确提供商、批次和编号独立补充，不重新分页匹配；批次依次读取，同一批次轨迹扫描一次。单个批次失败只标记对应调用，不丢弃成功结果或阻止后续批次；重试保留当前列表快照的成功结果，仅重新读取失败批次。字符数加载或失败不阻塞列表、翻页及打开明细。切页取消客户端旧请求，不将旧页结果合入新页；列表刷新会重新读取字符数，服务端轨迹扫描仍有开销，不重建响应输出。
...
未指定提供商或批次的入口按接口默认范围读取，收到结果后不自动改写 URL 或重复请求；打开记录及原始事件翻页使用返回的精确提供商和批次。调用列表手动刷新保留表头并显示骨架行。
...
调用详情顶部的模型名称旁显示 `routing.finalProvider` 对应的“上游”标签；字段缺失时不显示，也不从备用提供商推断。
...
HTTP/SSE 请求也从不携带，因此这些项在默认配置下显示“未提供”。
...
请求明细、会话列表、会话详情和错误页采用单行筛选工具栏：关键词搜索、筛选、查询和重置保持同一行；宽度充足时显示时间范围入口，窄屏将时间选择收进筛选面板。其他条件统一放在 Sheet 抽屉中，筛选按钮显示已选条件类别数（不含关键词）。关闭抽屉保留草稿，查询后应用，重置立即清除条件；自定义日期未填写完整或先后顺序无效时不发起查询。
...
缓存 Token 与命中率。控制台输入卡、期间统计的缓存量及 Threads 命中率剔除输入或缓存字段缺失的请求，明确返回缓存为零的请求仍参与；总请求数与总输入 Token 不受剔除影响。页面只展示缓存数值与命中率，不附加样本说明；全部缺失时显示 `—`。这些汇总通过 `cacheUsage` 返回已知样本的缓存合计、对应输入合计和缺失请求数，不用全量输入作命中率分母；原有 `cachedInputTokens` 字段仍保留完整性语义。用量趋势图使用输入、缓存、输出三项口径：输入与缓存共用左轴，输出使用右轴独立刻度，
...
同步失败不丢弃已取得的数据。账户列表或快照整体读取失败在账户区标题下提示；某一提供商的账户元数据无法读取时
...
请求、会话和轮次列表首次加载显示页面骨架；后续查询加载保留表格组件与表头，在表内显示骨架行并暂停表内操作，旧汇总和说明隐藏但保留占位，轮次累计区域保留展开状态。查询失败显示错误，不把旧结果当作新查询结果。共享表格为工具栏和分页保留实际高度；空间充足时表内滚动，手机或低高度窗口不足以容纳卡片时由页面滚动承接，分页不会被卡片裁切。
查询错误提供按当前条件重试；Token、次数、耗时和速率列的表头与数字统一右对齐。窄屏在表内横向滚动，不强制模型或状态跨行。
错误页同样支持查询重试和表内加载；请求总数和成功率卡片位于筛选栏上方，复用控制台的紧凑统计卡片及响应式列宽，筛选栏不显示 Thread ID / Turn ID 输入。加载时保留统计卡片与各行的占位尺寸，以骨架遮蔽旧值，避免翻页期间页面收缩导致滚动位置跳动；按时间、Provider、模型、状态、HTTP、错误明细、会话/轮次排列，长错误消息与错误码可聚焦查看。
移动端点击侧栏导航后自动关闭抽屉，桌面侧栏展开状态不受影响。
...
开始时间，不等同于 App Server 中 Thread 对象的创建时间。Threads 默认展示“缓存命中率”列，按当前筛选范围内该会话自身输入与缓存数据均已提供的请求计算：缓存 Token 合计 / 同批样本输入 Token 合计，保留一位小数；没有有效样本或样本输入为零时显示 `—`，该列不支持排序。Threads 的“类型”列把已由

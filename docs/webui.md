...
双向帧，「返回列表」回到原来的标签与页码。正文按顶层字段折叠，只显示字段名与体积，展开后先渲染
前 64 KiB，可继续加载；事件与帧同样按字段折叠，因此几 MB 的请求体不会一次性进入页面。转储把单个
条目截断时对应位置显示 `{"type": "truncated", …}` 标记（见
[`转储体积控制`](user-guide.md#转储体积控制)）。单段正文超过 4 MiB 时只返回前 4 MiB，并在响应里带
`bodyTruncated`。`label` 只接受 `traffic/` 中已存在的标签，非法或无来源的取值分别返回 400 与 404；
目录中还没有转储文件、或请求不来自回环地址时返回 503，响应里的 `enabled` 表示

...
部署 v18 前先停止 Gateway 并关闭独立 WebUI 数据库读取进程，运行 `codexc metrics upgrade`，命令会创建旧库私有备份并事务升级；
旧记录的总耗时保持 NULL；v17 已有调用关联保留，更早版本未记录的关联保持 NULL，已有首内容、上游 TTFT 和请求/响应模型名称保留。
...
悬浮查看完整值，Schema v13 起入库，当前 Schema v18 继续保留，早期历史记录显示 `—`）；请求明细的列排序作用于所选时间范围的全部记录，再由服务端偏移

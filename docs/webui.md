...
耗时摘要分为“本次调用”和“上游轮次统计”，CLI 调用详情采用相同分组。新调用响应索引的可选 `callTiming` 保存同一次调用的单调时钟偏移：HTTP 入口或 WS 请求帧接收入口为零点，记录转发开始、HTTP 请求体收齐/响应头、WS 提交发送、首字事件和结束。首字节点由发送起点加首内容耗时还原：HTTP 使用转发节点，WS 使用提交发送节点，不从 trace 反推。
...
诊断分段的总耗时仍覆盖代理入口至结束，包含发送前准备，与速率使用的请求总耗时区分；结束节点优先使用发送偏移加首次模型终态的请求总耗时还原；无终态时为上游结束或观察失败的时间，不包含其后客户端收到全部内容的耗时。历史调用没有 `callTiming` 时保留原墙钟耗时并明确未记录新阶段；无效或倒序节点不计算对应差值，不混用墙钟与单调时钟。V2 记录仅增加可选字段，原记录无需改写，回滚程序可忽略该字段。用于速率的请求总耗时独立经指标 IPC 保存到 Schema v18，诊断分段只保存在调用记录；需加载新版本 App Server 采集端才能产生节点。

调用详情、调用列表、请求明细列表与错误列表都在模型名称旁显示 `routing.finalProvider` 对应的“上游”小标签，不单列一列；字段缺失或只有备用提供商记录时都不显示，也不从备用提供商推断。调用列表与调用详情顶部标签统一读取响应索引的 `upstreamProvider`，共用模型标签组件；详情中的完整诊断仍读取 trace，只有 trace 诊断而没有索引字段的记录不补推顶部标签；调用记录的响应索引因此新增可选字段 `upstreamProvider`：只在同一次调用的 `chat_diagnostics` 明确记录过该值时写入，旧记录无需改写，读取端缺失即省略。请求明细列表与错误列表在读取时按记录的 `traffic` 只关联本页出现的批次索引、不读正文，批次较多时按需读取对应清单；调用记录缺失、批次已清理或版本不支持时只是不显示该标签，不阻断指标页、不回填历史，也不改变请求导出的字段。CLI `codexc traffic` 的调用列表同样在“模型”后显示 `上游=`（缺失为 `-`），`--exchange` 与 `--all` 的详情另起一行“上游提供商：”，缺失为“未记录”。
...
OCG 与 Cline Pass 的 5 小时、7 天和月度窗口还显示本机观测的 Token 换算：每消耗 1 个百分点约对应多少 Token，以及按该比例外推的满额 Token。它不是官方固定兑换率或剩余可用 Token 保证。
估算按精确账户 Provider、窗口 ID 和重置时间隔离，使用指标库保留的最近最多 2048 条官方账户快照及其间本地请求的输入加输出 Token（缓存输入已包含在输入中，不另加）。请求按实际开始时间归属区间，不按指标落库时间归属；已完成请求迟到落库后会重新参与计算，跨越快照边界的请求无法准确拆分额度消耗，其涉及区间不参与估算。第一次观测建立基线，百分比不变时继续累计，出现正向变化后闭合区间；结果为有效区间 Token 总和除以消耗百分点总和，不平均各区间比例。尚未闭合的尾部请求不参与换算；缺失 Token 的区间或没有本地 Token 的额度增长不用于估算。重置时间变化、比例倒退或无有效订阅等无效快照会重新建立基线，过期窗口不展示旧估算。
快照来自已有账户查询：使用一段时间后再次刷新额度，才能形成新的观测区间；不增加后台轮询。服务重启后可从保留历史重新计算；检测到请求历史前缀被清理时，只从最早保留明细的落库边界之后的账户快照重建基线，避免用残缺明细推算完整额度变化。不修改数据库格式或回填请求。界面区分正在采样、有效估算和指标读取暂不可用；后者不影响官方额度。有效估算同时显示观测百分点、区间数和请求数。其他设备的用量、模型与缓存比例、额度接口延迟及本机记录覆盖都会影响结果。

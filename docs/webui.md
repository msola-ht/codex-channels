...
调用列表在总耗时后显示“Turn State 字符数”：不同长度用 `/` 分隔，不累加，缺失显示 `—`，提示中列出来源。只为当前页解析对应调用的终态元数据及轨迹，同一批次轨迹扫描一次，不重建响应输出；因此大批次的列表读取仍有正文读取及轨迹扫描开销。
...
同一提供商、批次与调用编号的原始事件翻页保留已展开的详情区域，加载或失败时不展示旧页事件；切换调用不沿用旧摘要。首次进入读取完整明细，之后翻页使用 `GET /api/v1/traffic/trace?id=&label=&session=&traceOffset=`，只返回调用编号、事件页与分页信息，不重读正文或重新提取输出、模型证据；仍扫描轨迹以计算事件总数。列表和详情查询错误均可按当前条件重试，详情手动刷新及失败重试会重新读取完整明细。

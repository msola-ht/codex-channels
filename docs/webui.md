...
| 页面 | 路由 | API |
| --- | --- | --- |
| 概览 | `#/` | `GET /api/overview?range=24h\|7d\|30d` |
| Threads | `#/threads` | `GET /api/threads` |
| Thread 详情 | `#/threads/:id` | `GET /api/threads/:id/run`、`GET /api/threads/:id/turns` |
| 请求明细 | `#/requests` | `GET /api/requests?range=&afterId=&limit=` |
| 错误 | `#/errors` | `GET /api/errors?range=` |
...
```text
webui/src/
  lib/         API 客户端、类型与格式化（价格/Token/耗时）

...
本文定义 Codex CLI `0.148.0` 引入、并在当前锁定 `0.155.1` 复核的 `account/usage/read.threadId` 与 `threadUsage` 在 Gateway
...
- [`account_processor.rs`](https://github.com/openai/codex/blob/rust-v0.155.1/codex-rs/app-server/src/request_processors/account_processor.rs)：
  只接受规范 Thread UUID，只允许 ChatGPT 后端认证，将后端 `403` / `404` 收敛为
  `threadUsage: null`，超时和其他错误返回 JSON-RPC 错误。
- [`account_thread_usage.rs`](https://github.com/openai/codex/blob/rust-v0.155.1/codex-rs/app-server/tests/suite/v2/account_thread_usage.rs)：
...
不得声称套餐一定不支持，也不得建议切换 Provider、重新登录或充值，因为当前锁定的 `0.155.1` 合同没有提供
不可用原因。
...
- `account/usage/read` 已是现有稳定方法；本项目只采用它从 0.148.0 起提供、并在当前 0.155.1 复核的可选参数和响应字段，
  不增加实验握手或运行时兼容层。
...
6. 真实 App Server 合同：在条件式 `RUN_CODEX_INTEGRATION=1` 环境验证精确 0.155.1 接受规范

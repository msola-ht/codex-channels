...
微信客户端的一张图片就是一条独立消息。需要把多个独立图片消息作为同一次输入时，先发送：

```text
/vision begin 比较这些图片的差异
```

随后逐张发送最多四张图片，渠道会逐张确认已收集数量；最后发送 `/vision done` 一次提交。
`/vision cancel` 会丢弃尚未提交的收集。手动收集同样按 Actor 与 Conversation 隔离，仅保存在

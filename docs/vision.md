...
微信客户端的一张图片就是一条独立消息，因此一次 `/vision` 实际只作用于下一张图片；需要连续
定向识别多张独立图片时，应在每张图片前重新发送要求。Telegram 原生相册和飞书同一富文本多图
仍可作为下一批图片一次消费要求。
...
- `/vision` 待处理要求不写入 SQLite、配置或日志，不跨 Actor、Conversation 或 Gateway 重启恢复。

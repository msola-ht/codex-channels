...
| Thread 设置 | `thread/start.modelProvider`、`thread/start.config`、`thread/settings/updated`、`model/list`、`config/read`、`config/batchWrite`、实验 `collaborationMode/list` | [`model-port.ts`](../src/application/model-port.ts) 与 [`collaboration-mode-port.ts`](../src/application/collaboration-mode-port.ts) 定义稳定设置边界，[`model-adapter.ts`](../src/codex-client/model-adapter.ts) 映射 App Server 目录，[`deepseek-catalog.ts`](../src/codex-client/deepseek-catalog.ts) 映射 Setup 下载的官方 DeepSeek 目录；Flash 可选择，Pro 保留为明确不可用且不可切换的展示项；跨 Provider 选择保留旧 Thread 并用精确 Provider/目录新建 Thread，不在已有 Thread 的 Turn 中替换 Provider；[`notification-adapter.ts`](../src/codex-client/notification-adapter.ts) 隔离模型、Fast 与 Default/Plan 设置通知；[`json-rpc.test.ts`](../tests/json-rpc.test.ts)、[`model-selection-service.test.ts`](../tests/model-selection-service.test.ts)、[`deepseek-catalog.test.ts`](../tests/deepseek-catalog.test.ts)、[`deepseek-setup.test.ts`](../tests/deepseek-setup.test.ts)、[`collaboration-mode-service.test.ts`](../tests/collaboration-mode-service.test.ts)、[`notification-adapter.test.ts`](../tests/notification-adapter.test.ts) |

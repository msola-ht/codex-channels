...
官方页面全部模型的价格、时段与周末规则、端点和 SDK 协议，可选模型以下载的官方目录为准，价格基线覆盖目录模型与其仍可调用的旧模型名，
按需启动、初始化与模型列表流程由 Codex 0.153.4 真实 App Server 合同测试覆盖。`/usage` 与 WebUI
...
归属（缺失时由官方 `resetsAt` 倒推窗口）执行，`/usage` 按模型与 Peak/Off-Peak 档位展示本地
Token，不展示费用估算。

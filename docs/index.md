...
| 32 | 本项目直接调用的业务 Request 方法，不含连接层的 `initialize` | [`client.ts`](../src/codex-client/client.ts) |
...
| Skill、MCP 与 Plugin | `skills/list`、`turn/start` / `turn/steer` 的 `input.skill` 和 `mention`、`mcpServerStatus/list`、`mcpServer/oauth/login`、`mcpServer/resource/read`、MCP 状态通知、开发中 `plugin/installed` | Skill、MCP 与 Plugin 分别由 [`skill-port.ts`](../src/application/skill-port.ts)、[`mcp-port.ts`](../src/application/mcp-port.ts)、[`plugin-port.ts`](../src/application/plugin-port.ts) 及对应 Client 适配器隔离。Skill 继续按当前 CWD 重新解析已启用的用户或项目项。MCP 按当前 Thread 提供分页概览、完整工具/资源/模板详情、OAuth 登录和有界只读资源展示，不暴露直接 Tool Call；详情说明允许官方多行文本，在 Client 边界归一化空白、拒绝危险控制字符并限为 2,000 字符，三个 Surface 共用工具/资源/模板的每页 8 项分页搜索和前后页命令。OAuth 操作复用 Application 认证能力判断，不对 Bearer Token 或明确不支持的 Server 显示。Plugin 只在默认开启、可显式关闭的开发中开关下通过统一 `/plugin` 命令查询当前 Workspace 已安装项或调用，并仅对 OpenAI Thread 发送官方 `mention`；飞书命令中心只把已启用且可调用项映射为一次性选择和任务表单。搜索、市场、安装、卸载和分享仍禁止。三个 Surface 共用命令解析与输出，不维护渠道私有状态；[`conversation-service.test.ts`](../tests/conversation-service.test.ts)、[`conversation-command-service.test.ts`](../tests/conversation-command-service.test.ts)、[`conversation-command-format.test.ts`](../tests/conversation-command-format.test.ts)、[`json-rpc.test.ts`](../tests/json-rpc.test.ts)、[`provider-routing-client.test.ts`](../tests/provider-routing-client.test.ts)、[`real-app-server.test.ts`](../tests/real-app-server.test.ts)、[`module-boundaries.test.ts`](../tests/module-boundaries.test.ts) |
...
| 真实合同 | Fast 默认值、Skill/MCP/Plugin/Permission 稳定查询、结构化 Skill 与 Plugin mention Turn 输入、MCP 完整详情与只读资源、MCP 工具审批元数据与持久范围往返、Default/Plan 预设与 Plan Turn 设置通知、共享 Thread 设置通知、跨客户端 Thread 固定状态、Turn 启动结果、跨客户端 Goal 请求与通知、重连后 resume Goal 恢复、双客户端连接恢复 | [`real-app-server.test.ts`](../tests/real-app-server.test.ts) |
...
| MCP 状态、OAuth 与资源读取如何隔离 | [`mcp-port.ts`](../src/application/mcp-port.ts)、[`mcp-adapter.ts`](../src/codex-client/mcp-adapter.ts) | [`conversation-service.test.ts`](../tests/conversation-service.test.ts)、[`conversation-command-service.test.ts`](../tests/conversation-command-service.test.ts)、[`json-rpc.test.ts`](../tests/json-rpc.test.ts)、[`real-app-server.test.ts`](../tests/real-app-server.test.ts) |

...
| Skill、MCP 与 Plugin | `skills/list`、`turn/start` / `turn/steer` 的 `input.skill` 和 `mention`、`mcpServerStatus/list`、`config/mcpServer/reload`、`mcpServer/oauth/login`、`mcpServer/oauthLogin/completed`、`mcpServer/resource/read`、MCP 状态通知、开发中 `plugin/installed` | Skill、MCP 与 Plugin 分别由 [`skill-port.ts`](../src/application/skill-port.ts)、[`mcp-port.ts`](../src/application/mcp-port.ts)、[`plugin-port.ts`](../src/application/plugin-port.ts) 及对应 Client 适配器隔离。Skill 继续按当前 CWD 重新解析已启用的用户或项目项。MCP 按当前 Thread 提供分页概览、完整工具/资源/模板详情、只列需处理项与提示的健康摘要、OAuth 登录和有界只读资源展示，不暴露直接 Tool Call；`config/mcpServer/reload` 向全部受管 Provider App Server 发起配置刷新，已加载 Thread 在下一次活动 Turn 生效，任一实例失败时整体报错。详情说明允许官方多行文本，在 Client 边界归一化空白、拒绝危险控制字符并限为 2,000 字符，三个 Surface 共用工具/资源/模板的每页 8 项分页搜索和前后页命令。OAuth 操作复用 Application 认证能力判断，不对 Bearer Token 或明确不支持的 Server 显示；手工对 Bearer Token Server 调用登录时返回结构化认证结果，不进入渠道失败日志；官方完成通知经 Client 脱敏、Core 按 Thread 或 Provider 路由，再由三个 Surface 共用成功或失败提示。Plugin 只在默认开启、可显式关闭的开发中开关下通过统一 `/plugin` 命令查询当前 Workspace 已安装项或调用，并仅对 OpenAI Thread 发送官方 `mention`；飞书命令中心只把已启用且可调用项映射为一次性选择和任务表单。搜索、市场、安装、卸载和分享仍禁止。三个 Surface 共用命令解析与输出，不维护渠道私有状态；[`conversation-service.test.ts`](../tests/conversation-service.test.ts)、[`conversation-command-service.test.ts`](../tests/conversation-command-service.test.ts)、[`conversation-command-format.test.ts`](../tests/conversation-command-format.test.ts)、[`notification-adapter.test.ts`](../tests/notification-adapter.test.ts)、[`conversation-core.test.ts`](../tests/conversation-core.test.ts)、[`provider-routing-client.test.ts`](../tests/provider-routing-client.test.ts)、[`surface-copy-contract.test.ts`](../tests/surface-copy-contract.test.ts)、[`real-app-server.test.ts`](../tests/real-app-server.test.ts)、[`module-boundaries.test.ts`](../tests/module-boundaries.test.ts) |

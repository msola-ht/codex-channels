...
| Thread 设置 | `thread/start.modelProvider`、`thread/start.config`、`thread/fork.modelProvider`、`thread/fork.config`、`thread/settings/updated`、`model/list`、`config/read`、`config/batchWrite`、实验 `collaborationMode/list` | [`model-port.ts`](../src/application/model-port.ts) 与 [`collaboration-mode-port.ts`](../src/application/collaboration-mode-port.ts) 定义稳定设置边界，[`model-adapter.ts`](../src/codex-client/model-adapter.ts) 映射 App Server 目录，[`deepseek-catalog.ts`](../src/codex-client/deepseek-catalog.ts) 映射 Setup 下载的官方 DeepSeek 目录；切换模式只在基础配置登记 App Server 必需的 DeepSeek Provider，CLI 模型选择保留在独立 Profile，App Server 本身不使用 `--profile`；官方 Remote 模式会省略本地 `modelProvider`，因此 `codexc remote` 明确拒绝 DeepSeek Profile，只允许恢复已经由渠道切换生成的 DeepSeek Thread；Flash 可选择，Pro 保留为明确不可用且不可切换的展示项；跨 Provider 选择不在已有 Thread 的 Turn 中替换 Provider，而是通过官方 `thread/fork` 复制当前历史并以精确 Provider/目录重绑当前 Conversation，Fork 失败时保留原绑定；切回 OpenAI 时不传自定义目录，让 Fork 从用户默认配置重新加载官方目录；[`notification-adapter.ts`](../src/codex-client/notification-adapter.ts) 隔离模型、Fast 与 Default/Plan 设置通知；[`json-rpc.test.ts`](../tests/json-rpc.test.ts)、[`model-selection-service.test.ts`](../tests/model-selection-service.test.ts)、[`session-router.test.ts`](../tests/session-router.test.ts)、[`deepseek-catalog.test.ts`](../tests/deepseek-catalog.test.ts)、[`deepseek-setup.test.ts`](../tests/deepseek-setup.test.ts)、[`collaboration-mode-service.test.ts`](../tests/collaboration-mode-service.test.ts)、[`notification-adapter.test.ts`](../tests/notification-adapter.test.ts) |

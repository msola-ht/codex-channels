...
| MCP 状态测试 | [`mcp_server_status.rs`](https://github.com/openai/codex/blob/rust-v0.150.1/codex-rs/app-server/tests/suite/v2/mcp_server_status.rs) | 工具原名、项目级配置、实时元数据、当前 Thread 连接状态、断线失败与精简清单合同 |
...
| MCP 配置刷新测试 | [`executor_mcp.rs`](https://github.com/openai/codex/blob/rust-v0.150.1/codex-rs/app-server/tests/suite/v2/executor_mcp.rs) | 从磁盘重载配置并刷新已加载 Thread 的 MCP 运行时；请求成功不等于远端握手已经完成 |
...
| 本机 App Server Transport 如何按平台连接并对齐原生 128 MiB 消息上限 | [`app-server-transport.ts`](../src/codex-client/app-server-transport.ts) 在 macOS/Linux 选择 [`unix-websocket-transport.ts`](../src/codex-client/unix-websocket-transport.ts)，在 Windows 选择拥有官方 `app-server proxy --sock` 子进程的 [`windows-proxy-transport.ts`](../src/codex-client/windows-proxy-transport.ts)；完整边界见 [`codex-client/`](../src/codex-client/README.md) | [`unix-websocket-transport.test.ts`](../tests/unix-websocket-transport.test.ts)、[`real-app-server.test.ts`](../tests/real-app-server.test.ts)、[`windows-app-server-proxy-probe.mjs`](../scripts/windows-app-server-proxy-probe.mjs)、[`windows-proxy-inbound-limit-probe.mjs`](../scripts/windows-proxy-inbound-limit-probe.mjs) |

...
| Skill、MCP 与 Plugin | `skills/list`、`turn/start` / `turn/steer` 的 `input.skill` 和 `mention`、`mcpServerStatus/list`、`mcpServer/oauth/login`、`mcpServer/resource/read`、MCP 状态通知、开发中 `plugin/installed` | Skill、MCP 与 Plugin 分别由 [`skill-port.ts`](../src/application/skill-port.ts)、[`mcp-port.ts`](../src/application/mcp-port.ts)、[`plugin-port.ts`](../src/application/plugin-port.ts) 及对应 Client 适配器隔离。Skill 继续按当前 CWD 重新解析已启用的用户或项目项。MCP 按当前 Thread 提供分页概览、完整工具/资源/模板详情、OAuth 登录和有界只读资源展示，不暴露直接 Tool Call；详情说明允许官方多行文本，在 Client 边界归一化空白、拒绝危险控制字符并限为 2,000 字符。Plugin 只在默认开启、可显式关闭的开发中开关下通过统一 `/plugin` 命令查询当前 Workspace 已安装项或调用，并仅对 OpenAI Thread 发送官方 `mention`；搜索、市场、安装、卸载和分享仍禁止。三个 Surface 共用命令解析与输出，不维护渠道私有状态；[`conversation-service.test.ts`](../tests/conversation-service.test.ts)、[`conversation-command-service.test.ts`](../tests/conversation-command-service.test.ts)、[`conversation-command-format.test.ts`](../tests/conversation-command-format.test.ts)、[`json-rpc.test.ts`](../tests/json-rpc.test.ts)、[`provider-routing-client.test.ts`](../tests/provider-routing-client.test.ts)、[`real-app-server.test.ts`](../tests/real-app-server.test.ts)、[`module-boundaries.test.ts`](../tests/module-boundaries.test.ts) |

...
| 子代理活动与终态 | `subAgentActivity`、`item/completed` 的 `collabAgentToolCall.receiverThreadIds` 与 `agentsStates` | [`operation-adapter.ts`](../src/codex-client/operation-adapter.ts) 把官方接收线程和状态映射为不含代理消息正文的稳定操作事件，[`subagent-completion-tracker.ts`](../src/bootstrap/subagent-completion-tracker.ts) 登记生成的子代理线程，并只在官方完成、异常、中断、关闭或未找到终态后进入有界指标收敛窗口、等待 Writer 当前持久化水位，再向父会话发布一次关键终态事件；等待期间的新指标使旧结算失效，指标到达和静默时间不推断终态。无指标发布零统计，读取失败显示统计不可用。三个 Surface 的紧凑操作模式共用同一策略，只保留子代理启动和失败，抑制成功的等待与交互操作；完成卡片展示模型、请求、Token、费用及全量计价均价，缓存、推理、费用分项和模型请求聚合耗时仅在调试模式展示。[`notification-adapter.test.ts`](../tests/notification-adapter.test.ts)、[`subagent-completion-tracker.test.ts`](../tests/subagent-completion-tracker.test.ts)、[`lifecycle-presentation.test.ts`](../tests/lifecycle-presentation.test.ts)、[`request-metrics-store.test.ts`](../tests/request-metrics-store.test.ts)、[`telegram-outbox.test.ts`](../tests/telegram-outbox.test.ts)、[`feishu-outbox.test.ts`](../tests/feishu-outbox.test.ts)、[`weixin-outbox.test.ts`](../tests/weixin-outbox.test.ts) |

...
| Item 与流式输出 | `item/started`、`item/completed`、`item/agentMessage/delta` | [`notification-adapter.ts`](../src/codex-client/notification-adapter.ts) 分类稳定 Item 事件，[`operation-adapter.ts`](../src/codex-client/operation-adapter.ts) 生成脱敏操作摘要，并只把官方 `imageGeneration.savedPath` 映射为生成图片产物，[`core.ts`](../src/conversation-core/core.ts) 只归约稳定输入；输出速度由 Client 在通知边界记录接收时间戳、Core 按 Thread 归约最后一次模型响应的非推理输出 Token 与最终回答流式时长，不消费新协议字段；[`generated-image.ts`](../src/surfaces/generated-image.ts) 对三渠道共用的生成图片本地读取执行绝对路径、无符号链接、普通文件、10 MiB 与 PNG/JPEG 签名校验；[`notification-adapter.test.ts`](../tests/notification-adapter.test.ts)、[`operation-adapter.test.ts`](../tests/operation-adapter.test.ts)、[`conversation-core.test.ts`](../tests/conversation-core.test.ts)、[`telegram-outbox.test.ts`](../tests/telegram-outbox.test.ts)、[`feishu-outbox.test.ts`](../tests/feishu-outbox.test.ts)、[`weixin-outbox.test.ts`](../tests/weixin-outbox.test.ts) |
...
| DS 模型代理与速度统计 | 配置 `ds_proxy.listen`（回环地址） | [`provider-proxy/`](../src/provider-proxy/README.md) 由 App Server 服务持有，流式转发 DeepSeek 精确 `/responses` 路径，保留上游状态与端到端 Header，剥离本地 `x-codex-turn-metadata` 后按其中 Thread/Turn 关联推理时间；[`bin/codexc.mjs`](../bin/codexc.mjs) 复用统一网络代理、启动回环代理，并用 [`withProviderBaseUrl`](../runtime/model-provider-runtime.mjs) 把 DeepSeek `base_url` 指向它；指标经 `0600` 私有 Unix Socket 投递并在首个可见输出或响应完成事件前确认归约，[`bootstrap/app.ts`](../src/bootstrap/app.ts) 只接收指标并转成 `turn.modelTiming.updated`。Core 使用最后一次模型请求的推理窗口计算思考速度（推理 token/s）与生成速度（含推理）；Gateway 停止时指标可丢失但模型请求不中断。仅支持 DeepSeek 切换模式，其他模式失败关闭，配置变化需要重装整套服务；[`provider-proxy.test.ts`](../tests/provider-proxy.test.ts)、[`provider-proxy-metrics.test.ts`](../tests/provider-proxy-metrics.test.ts)、[`conversation-core.test.ts`](../tests/conversation-core.test.ts)、[`model-provider-runtime.test.ts`](../tests/model-provider-runtime.test.ts)、[`codexc-cli.test.ts`](../tests/codexc-cli.test.ts)、[`config-reload.test.ts`](../tests/config-reload.test.ts) |

...
| 开发中 Plugin 已安装列表、详情与 mention 调用 | 调试当前 Workspace 已安装 Plugin，查看同一已安装响应中的版本、来源、安装时间和可用原因，并把选中项作为官方 mention 输入交给 Turn | `[experimental].plugin_api` 默认关闭且只在显式开启时可用，Doctor 和命令输出持续标记开发中；详情不调用其他 Plugin API，不展示来源路径、远端 URL 或原始 Marketplace 错误；只支持 OpenAI Thread，不接入搜索、安装、卸载或分享 | [`plugin-port.ts`](../src/application/plugin-port.ts)、[`plugin-adapter.ts`](../src/codex-client/plugin-adapter.ts)、[`turn-adapter.ts`](../src/codex-client/turn-adapter.ts)、[`conversation-service.test.ts`](../tests/conversation-service.test.ts) |

...
- [ ] Telegram、飞书和微信拆分为开始、状态、确认、取消。
  - [x] 微信已提供进程内会话接口，现有 CLI 已复用；Telegram 与飞书待处理。
- [ ] 二维码、OAuth 和等待消息具有所有者、超时与取消路径。
  - [x] 微信二维码、配对码等待和确认预览已绑定所有者、整体期限与真实网络中止；Telegram
    等待消息和飞书 Device Authorization 待处理。
- [ ] 保存前返回脱敏预览；凭据只在受控会话内传递。
  - [x] 微信状态只返回账号、扫码用户和凭据已获取标志，Bot Token 只由会话持有并在确认事务中写入；
    Telegram 与飞书待处理。

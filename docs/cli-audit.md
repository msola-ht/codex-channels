...
- 统一结果已消除大部分重启建议的语义差异；仍需由服务生命周期执行器补齐跨平台多目标部分失败后的统一结果。

### 测试覆盖

- `tests/setup.test.ts` 覆盖总菜单分类、返回/取消、模型与渠道子菜单委派和模块调用参数。
- 各 Setup 子模块有独立测试，覆盖输入验证、确认取消和部分配置写入。
- 未覆盖：跨模块部分成功后的统一摘要、服务未运行时的交互引导、Setup 完成后验证实际运行状态；各子模块的激活类别与可执行命令已有统一结果测试。

### 结论

Setup 的职责拆分和返回路径清楚，脱敏总览与确认机制符合边界；当前各子模块已经返回统一激活状态与命令，但 Setup 仍没有跨模块配置变更摘要。后续应保留模块边界，不把所有写入合并成大事务；若增加摘要，应只汇总本次已完成的子模块和未生效原因。
...
- 未覆盖：自动重启失败后的跨模块统一摘要、Config 与 Setup 的重复入口契约、`--json` 输出的文档边界、跨多个设置连续修改的部分成功摘要。

### 结论

Config 更适合日常运维，数据中心配置也已具备双令牌生成和自动重启闭环；配置菜单现在与写入器共享激活结果，失败时保留配置并返回待生效状态。后续重点是明确 Setup 负责首次/跨领域配置、Config 负责日常运行参数，减少重复入口，而不是再复制一套激活语义。

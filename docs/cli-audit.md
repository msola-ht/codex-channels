...
核心启动路径有较完整的配置校验和稳定就绪等待；目标语义也明确，`all` 不包含独立 WebUI/指标中心属于既有设计。独立服务现在对已配置/启用目标做健康等待，但仍需补真实平台失败路径和统一部分成功结果。
...
JSON 状态模型是目前最完整、最适合后续自动化的事实接口；普通文本仍受平台脚本历史行为影响，尤其 macOS 只判断 Job 是否加载，可能把未运行服务显示为正常。后续统一修复应让文本输出复用同一健康模型，并明确 `all` 是核心服务集合还是全部受管服务，避免与用户对 WebUI/指标中心的预期冲突。
...
- 公开入口：`codexc setup`，不接受参数；`-h/--help` 输出交互菜单说明。
...
- 大多数子模块写入后只输出后续生效命令，不由 Setup 自动执行服务重载/重启。

### 热重载与热重启影响

- Setup 菜单本身不触发热重载或热重启。
- 不同子模块的生效提示不完全一致：渠道配置通常提示 Gateway 自动重读或按需重启，Codex 默认值/第三方 Provider 常提示 `codexc service restart all` 或 `restart app-server`，涉及服务环境的设置要求重新安装服务。
- 运行中的 Gateway 可能自动重载配置并自行退出，由监管器重启；前台进程需要用户手动重启。Setup 不等待最终生效状态。
...
- 子模块文案中的重启建议存在语义差异，用户难以判断“只重载 Gateway”“重启 App Server”“重启全部”与“重新安装服务”的关系。

### 测试覆盖

- `tests/setup.test.ts` 覆盖总菜单分类、返回/取消、模型与渠道子菜单委派和模块调用参数。
- 各 Setup 子模块有独立测试，覆盖输入验证、确认取消和部分配置写入。
- 未覆盖：跨模块部分成功后的统一结果、所有子模块生效命令的一致性、服务未运行时的交互引导、Setup 完成后验证实际运行状态。

### 结论

Setup 的职责拆分和返回路径清楚，脱敏总览与确认机制符合边界；主要问题是它只是“多个独立写入器的菜单编排”，没有统一的配置变更摘要和激活状态。后续应保留模块边界，不把所有写入合并成大事务；但应统一每个子模块返回的变更类别（热加载/重启/重装/下次启动）和推荐命令，并在 Setup 结束时汇总本次修改及尚未生效原因。
...
- 自动重启失败不会回滚已写入配置；当前输出只提示手动重启，未返回统一的“配置已写入/服务未生效”结构化结果。
- `--json` 不是脱敏完整配置导出，只能用于定位路径；与交互菜单“配置总览”的能力边界容易被误解。
- Config 和 Setup 都能修改部分相同设置，但入口层级、文案和重启建议不同，存在重复入口及语义分裂。
...
- 未覆盖：自动重启失败后的统一状态对象、Config 与 Setup 的重复入口契约、`--json` 输出的文档边界、跨多个设置连续修改的部分成功摘要。

### 结论

Config 更适合日常运维，数据中心配置也已具备双令牌生成和自动重启闭环；主要问题是“配置写入”和“服务激活”仍是两个未统一的结果，失败后用户需自行判断当前状态。后续应统一返回激活状态与手动恢复命令，并明确 Setup 负责首次/跨领域配置、Config 负责日常运行参数，减少重复入口和重启文案分裂。

## Setup + Config 联合审查

### 共同覆盖范围

| 配置领域 | Setup | Config | 当前问题 |
| --- | --- | --- | --- |
| Gateway 基础/显示/权限 | 首次或跨领域设置 | 日常修改 | 激活分类和重启文案不完全一致 |
| 模型与 Provider | OpenAI、第三方 Provider、共享子代理 | 部分运行参数/数据中心关联项 | Provider 切换后通常只提示重启，不返回实际生效状态 |
| 通讯渠道 | 首次配置和恢复 | 日常渠道参数/格式 | 热加载、Gateway 重启、App Server 重启的提示口径分散 |
| 数据中心 | 由 Setup 进入 Provider/子代理流程 | 本机上报、查看令牌、中心服务、WebUI | 双令牌配置已分开，但配置写入和服务重启仍是两个结果 |
...
3. 每次写入都应返回统一激活结果：`无需处理`、`Gateway 热加载`、`Gateway 重启`、`App Server 重启`、`服务重装`、`下次启动生效`。
4. 自动重启成功时显示“配置已保存并已生效”；自动重启失败时显示“配置已保存但尚未生效”，同时给出精确恢复命令，不要只抛通用异常。
...
- Config、metrics 维护、Update 和平台脚本各自实现停启/重启；应集中到统一生命周期执行器。

...
- 运行中的 Gateway 可能自动重载配置并自行退出，由监管器重启；前台进程需要用户手动重启。Setup 不等待最终生效状态，且部分子模块没有返回结构化激活字段。
...
- 统一结果已消除大部分重启建议的语义差异；仍需由服务生命周期执行器补齐跨平台多目标部分失败后的统一结果，并由 Setup/CLI 公开输出该结果。
...
Setup 的职责拆分和返回路径清楚，脱敏总览与确认机制符合边界；底层激活结果已经抽取，但并非所有子模块返回该字段，且独立 CLI 不输出机器可读变更结果。后续应保留模块边界，只补齐返回契约和本次操作摘要，不把所有写入合并成大事务。
...
- Config 和 Setup 仍能修改部分相同设置，但底层写入器与激活结果已复用；独立 CLI 的机器可读变更结果、跨入口确认流程和部分子模块返回值仍未统一。
...
Config 更适合日常运维，数据中心配置已具备双令牌生成、自动重启和有界健康确认；配置菜单现在与写入器共享激活结果，失败时保留配置并返回待生效状态。Setup 的统一结果目前只在部分子模块和进程内调用可见，公开 `codexc setup` 不输出机器可读变更结果。后续应补齐子模块返回契约并统一服务生命周期结果，而不是再复制一套激活语义。
...
3. 每次写入都应返回统一激活结果：`无需处理`、`Gateway 热加载`、`Gateway 重启`、`App Server 重启`、`服务重装`、`下次启动生效`。当前 Config 写入器已返回该对象，但 Setup 的部分子模块和公开 CLI 尚未完整暴露。

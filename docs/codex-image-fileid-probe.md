...
- 早期在线探测：2026-09-23 UTC，当前 ChatGPT 账户；模型目录读取自当时仍运行的 `0.155.1`
  App Server。推理直接请求官方 HTTP 后端，未经过 Gateway，只作为上传编号与模型互通证据。
- 生产链路验收：同日部署提交 `0ed940a8`，Gateway 与 Codex CLI 均为 `0.156.1`；飞书正常发图
  经 ChatGPT 默认后端和 OpenAI 模型完成首次识图及后续纯文本追问。
...
本次验收使用飞书私聊正常图片消息，不调用单独上传命令，也不向用户暴露文件编号。模型请求转储
仅提取类型、接续方式和是否存在 Base64；真实编号、账户、Thread、Turn、响应编号、消息正文和
签名地址均不写入本记录。

| 调用 | 脱敏观测 | 结论 |
| --- | --- | --- |
| 首次发图 | `previous_response_id` 为空；`input` 中存在 `input_image.file_id`；没有 `image_url` 或 `data:image/...;base64`；模型成功描述图片 | Gateway 上传与官方编号提交生效 |
| 后续纯文本追问 | 仅携带本轮文本和上一轮 `previous_response_id`；没有图片字段或 Base64；模型仍能引用前图 | 官方 WebSocket 增量历史复用生效，没有重复发送图片 |

这两次调用来自同一 Thread 的相邻 Turn。首次请求证明图片表示已经从 Surface 的临时内联数据转换为
模型侧 `file_id`；后续回答本身不能证明编号表示，但与首次请求及 `previous_response_id` 共同证明
本次原生接续没有重新携带图片。若某次传输没有使用增量接续，完整历史中再次出现同一 `file_id`
也符合预期；验收失败条件是后续请求重新出现该图片的 Base64。

本结果只覆盖当前 ChatGPT 账户、默认后端、`NO_CONSTRAINT` 路由、飞书和当次 OpenAI 模型。
Telegram、微信、其他账户、区域路由、其他模型、编号过期及跨重启后的远端可用性没有由本次实机
验收推出。
...
扩大图片最初未获执行批准；用户随后授权继续，已完成第二次上传和两次早期直接 HTTP 推理。
该阶段服务尚未升级或重启；后续生产链路验收已使用 `0.156.1` Gateway 和 App Server，见上文。
...
| 路径 | 已有依据 | 剩余条件 |
| --- | --- | --- |
| ChatGPT Apps 文件 | 固定源码上传、即时下载、当前账户在线图片引用，以及 `0.156.1` Gateway 飞书端到端识图与增量复用成功 | 其他渠道/账户/路由/模型、认证刷新、独立取回及删除/过期合同 |
...
未新增 Platform 凭据配置或生产文件管理入口。Client 已为符合账户与路由约束的 OpenAI ChatGPT
图片执行自动上传并提交官方引用；API Key、第三方 Provider 和独立自定义 OpenAI 后端继续使用
内联图片。当前决定和剩余限制见[升级决策](codex-cli-upgrade-decisions.md#图片文件引用需求阻塞与实现边界)。

...
| UTF-8 文本文件输入 | `im.message.receive_v1`、`im.v1.messageResource.get` | 独立 `file` 消息在授权后按 `message_id + file_key + type=file` 下载；严格验证文件名、1,000,000 字节、UTF-8 与控制字符，内联提交且不落盘。Office、压缩包、音视频和富文本内附件不支持；离线主路径已完成，真实文件待验收 | 阶段 4 |

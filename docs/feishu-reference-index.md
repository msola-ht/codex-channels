...
| Thread 状态消息更新 | `client.im.v1.message.create/patch` | 同一 Thread 的 `active → idle` 轻量交互卡片创建、重复抑制、同 Chat 顺序更新、失败绑定清理和有限关闭已离线验证；蓝色运行中原地更新为绿色“处理结束 · 结果见下方消息”的真实主路径已通过，不更新模型正文且不自动重试 | 已接入 |

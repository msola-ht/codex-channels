...
| 卡片交互 | `im.v1.message.create/patch`、`card.action.trigger` | 私聊审批、CardKit 2.0 用户输入与 MCP form、MCP URL 卡片、一次性令牌、Actor/Chat/消息/请求绑定、重复请求失败关闭、原值决定、超时、有限停止、结果更新失败隔离和跨客户端失效已离线验证；用户输入使用原生选择器并允许单独填写“其他内容”，同时兼容动作值和 CardKit 表单按钮名两种回调载荷；命令审批卡片的一次批准与当前 Gateway 长连接动作接收已通过真实验收，用户输入和 MCP 卡片仍待真实验收 | 阶段 3 |

...
| 生成图片回传 | `im.v1.image.create`、`im.v1.message.create` 的 `msg_type=image` | 只接受官方 `imageGeneration.savedPath`，共享边界拒绝相对路径、符号链接、非普通文件、超过 10 MiB 和非 PNG/JPEG 内容；上传后严格裁剪 `image_key` 并按 Chat 顺序发送，独立于操作显示档位。离线 Payload、顺序和失败关闭已验证，真实发送待验收 | 阶段 4 |

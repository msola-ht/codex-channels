...
`resetsAt` 快照写入指标记录（Schema v9 新增 `quota_windows` 列；当前指标库 Schema v11 另保存子代理
...
同时在窗口下方展示模型本地用量估算：按本机指标库汇总当前官方月度窗口内每个模型/档位的本地
Token（输入与输出合计；窗口按请求记录的官方快照归属，缺失时由官方 `resetsAt` 倒推开始时间）。
请求档位优先沿用请求时保存的价格快照（`pricing_bucket`），快照缺失时按当前峰谷规则判定，
官方调整峰谷时段不会让历史请求漂移。官方表格把 DeepSeek 按 Off-Peak / Peak 拆成两行，
本地估算也按请求时间分档展示两行，其余模型按单档展示；模型本地用量行只显示 Token，
不显示费用估算。凭据按当前 Thread 的 `modelProvider` 读取对应账户的私有 Profile
...
模型本地用量估算。OpenCode Go 启动卡片会把指标中心当前周期的 5小时、7天、30天三个窗口分块展示；
Turn 完成通知展示额度中心已有的周期，5 小时窗口尚未同步时使用最近官方账户快照补齐；中心不可用时再附带当前模型剩余用量
（同一口径，DeepSeek 按该 Turn 最后一次模型请求开始时间的峰谷档位），官方用量接口或本地指标不可用时
自动省略。

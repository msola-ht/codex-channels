...
当前锁定的 Codex CLI 0.156.1 已支持多客户端连接同一 App Server；Codex Desktop App 当前构建还
...
- 项目固定 `codex-cli 0.156.1`，协议与实现仍以 [`Codex 协议索引`](index.md)和
  `upstream/openai-codex` 的 `rust-v0.156.1` 锁定源码为准。下文 0.154.0 的实机结果是历史验收，
  不代表 0.156.1 已完成打包 Desktop 私有工具 Pipe 与签名链的实机复核。
...
- Desktop 随包 Codex 为 `0.154.0-alpha.6.2`，与项目锁定协议基线不同，不能成为共享主 App
  Server。实施只能继续运行项目解析出的精确 Codex CLI 0.156.1 原生可执行文件；Desktop 随包内容
...
3. App Server 服务解析项目当前使用的 Codex CLI 原生可执行文件，要求版本为 0.156.1，并验证
...
10. macOS 受管路径必须继续使用项目锁定的 Codex CLI 0.156.1；不得以 Desktop 随包的预发布 CLI

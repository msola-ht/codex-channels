...
当前进度（2026-07-29）：已锁定官方 Node SDK `1.71.1`，并完成 Phase 0 的长连接生命周期窄封装、
...
MCP form/URL 卡片动作仍待真实验收；群聊切片继续暂停。原生用户输入卡和 MCP 工具审批的一次批准
已通过真实验收。

2026-07-26 操作者在 Gateway 重启后完成飞书私聊命令试用，确认当时回复均为纯文本。随后按独立
切片实现最终回复和命令结果的 `post + md` 富文本；当前展示型 Markdown 已进一步统一为
CardKit 2.0，`post + md` 只保留为卡片实体创建失败时的安全降级；错误、过载和操作性提示继续使用纯文本，
同日已在 Gateway 重启后通过真实飞书验证状态命令与普通 Turn 的短回复，标题、列表、加粗、
行内代码和链接均正确显示。群聊需求已记录为阶段 2 后续项且当前开发批次明确暂停；审批卡片、
一次性令牌、精确 Actor/Chat/消息/请求绑定、原值决定和失效更新已完成离线实现。随后操作者已
通过真实命令审批卡片完成一次批准，当前 Gateway 收到
`card.action.trigger` 后任务继续完成；私聊 PNG/JPEG 图片输入也已通过真实消息验收。长回复
在飞书客户端折叠显示且消息顺序正确；受限 UTF-8 文本文件输入已完成离线实现并等待真实验收，
Office、压缩包和其他任意一般文件不支持。Gateway 重启后的 OAuth Token 与
...
直接复用共享命令。分类入口再按会话查询、会话操作、能力与集成、当前内容、飞书展示二十一个按钮，不发送
完整帮助长文本。会话切换、会话列表、已归档、模型、思考强度、Fast 与工作区使用有界选择卡，
选择值与当前卡片令牌精确绑定并继续通过共享命令服务执行；会话与归档搜索、重命名、下一 Turn
追加、Review 和 Goal 复用一个有界输入卡片，项目规则只提供共享命令已有的安全动作。选择卡与
输入卡一次性使用，命令名和语法直接复用 Application 的统一目录与解析，不建立飞书专属状态或队列。
主卡与分类卡的只读查询保持可复用；直接新建、停止、归档、固定、取消固定、压缩和分叉会话会
消费当前卡片令牌，
避免平台重投或重复点击执行第二次写操作。
菜单事件仅在 Actor 精确匹配唯一已授权私聊时路由，
事件去重和卡片令牌均为有界内存状态。Doctor 区分菜单节点和启用开关；官方授权后只重新检测并
由 Owner 在飞书客户端内确认授权后，Gateway 自动补齐菜单、事件与回调并提交应用版本；已有
待发布版本时不覆盖，仍由 Owner 或管理员处理。
...
├── command-center.ts # 分类命令与有界选择卡、短期令牌、精确参数绑定及菜单事件去重
...
4. 明确展示准备写入的非敏感配置，成功后原子更新 TOML；扫码方式随后保留已有菜单并自动发布
   `codexc_home` 悬浮菜单、长连接菜单事件与卡片回调，失败时保留连接配置并提供 Doctor 恢复。
5. 手动凭据方式不直接修改远端应用；不自动扩大允许用户或群聊范围。
...
- [x] 所有关键 `OutputEvent` 的纯文本回退，以及 Client 边界脱敏限长后的上游错误详情展示；
...
测试；阶段 1 的功能实现已经完成，未授权/重复真实事件仍是验收缺口。后续私聊能力可以按独立
切片推进，但不能据此把群聊或未经真实验证的路径标记为公开支持。
...
群聊只记录需求，不在当前开发批次实施。开始群聊前仍须单独确认群 Chat 配置、允许名单、
Actor 联合授权、Bot 身份和 `@Bot` 触发边界。
...
当前私聊命令实现已经完成并可独立审查。Gateway 重启绑定恢复、命令中心、分类卡、状态、主要
...
卡片申请精确差集。菜单或菜单事件缺失时，Owner 可通过同一卡片授权
`application:application:patch`，Gateway 保留已有菜单并自动启用 `codexc_home`、追加长连接
菜单事件和提交应用版本；存在待发布版本时拒绝覆盖。授权结果必须匹配当前 App 和飞书租户。
...
验收，原生用户输入卡和 MCP 工具审批的一次批准也已通过真实验收；MCP form/URL 卡片动作仍属于
本阶段未完成验收项。
...
第二个体验切片最初只接受已授权私聊的 `image` 事件。SDK 回调同步裁剪 `image_key` 并进入现有
...
Application 图片输入。后续已补齐官方 `post` 中最多四张图片与可选说明文字，并在同次
Application 提交中保留原始顺序和说明文字；一般文件、群聊媒体和下载重试仍不在本切片范围。
真实飞书独立 PNG/JPEG 图片消息及两图加说明文字的单 Turn 主路径已经通过验收，三至四图上限
仍待复验；大小超限、无效签名等失败路径仍由离线测试覆盖。
...
第六个体验切片把启动通知、短回复、命令结果和每轮结束统计统一为 CardKit 2.0 静态 Markdown；
只有卡片实体创建失败且尚未向 Chat 发送时才安全降级为 `post + md`，消息创建失败后不改发，
避免非幂等重复。静态卡片沿用 5,000 个 Unicode 字符单元素和最多 5 张卡片的有界分片。同一
Turn 的 `operation.updated` 运行中间帧不输出，完成、失败或拒绝终态按事件到达顺序进入同一
Chat 队列，以独立静态卡片显示 Core 已提供且已脱敏、截断的命令、文件、工具、搜索等摘要。
这样工具卡片与前后助手消息保持真实会话顺序，不因原地更新旧卡片而混排。该切片不读取原始
协议对象、不持久化命令正文或消息 ID，已完成离线验证，仍待真实飞书验收。
...
之后已完成第 6 项中的命令审批和用户输入主路径。断线恢复、第 4 项及第 6 项的审批超时与
跨客户端失效仍未完成真实验收；该记录不包含真实消息、应用标识、Chat ID 或凭据。
...
5. 真实测试应用已接受 Setup 当前声明的权限与授权流程。扫码 Setup 与 Doctor 必须准确区分
   `codexc_home` 节点和“已添加，尚未启用”，并只通过已核对的 Application v7 字段受控发布；
   不猜测参数、不重复授权、不覆盖已有待发布版本。单菜单入口的发布、点击和分类卡片显示已经完成真实验收；当前私聊
   Surface 不接受为后续媒体或 Drive 预授予宽泛权限。

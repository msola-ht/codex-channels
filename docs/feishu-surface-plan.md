...
截至 2026-07-25，已授权私聊 Turn 和文本回复已经通过真实测试；未授权/重复真实事件和 Gateway
重启后的 Thread 绑定恢复仍待验收，因此阶段 1 尚未整体关闭。
...
Codex 输入。Outbox 对纯文本采用项目内部的 20,000 UTF-8 字节单条上限并按逻辑结果顺序分片，
每个逻辑结果最多 5 条，超出时明确标记截断；会话列表最多展示 20 条并规范、截断长预览。
这是阶段 2 命令可靠性所需的最小传输边界，不提前引入阶段 4 的富文本、文件回退或消息更新。
...
基础纯文本 UTF-8 有界安全分片已经作为命令传输边界完成；本阶段的“长回复”指富文本布局、代码
文件回退、消息更新和经真实限制验证后的体验优化，不再重复实现基础分片。

...
提示 Owner 在开放平台人工配置、确认事件/回调和发布版本，Gateway 不自动写入或发布应用配置。
...
5. 真实测试应用已接受 Setup 当前声明的权限与授权流程。Doctor 必须准确区分 `codexc_home`
   节点与“已添加，尚未启用”，并提供人工配置入口；Gateway 不通过猜测参数、重复授权或应用
   写接口尝试修改或发布。单菜单入口的发布、点击和分类卡片显示已经完成真实验收；当前私聊

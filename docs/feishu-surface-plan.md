...
只读动作。真实测试应用的已发布版本已有 `codexc_home` 节点，但 `bot_menu_enable=false`；
Doctor 已改为区分节点和开关；官方授权后只重新检测并提示 Owner 在开放平台人工启用菜单、
确认事件/回调和发布版本，Gateway 不自动写入或发布应用配置。菜单启用、点击和结果显示仍待验收。
选择器、表单与写操作确认不在本切片。
...
- [x] 同一 Turn 操作状态合并为一张 CardKit 进度卡并显示受控详情；
- [x] 富文本真实应用短回复显示验收；
...
Turn 的 `operation.updated` 按 Item ID 保存最新状态并合并到一张流式进度卡，显示 Core 已提供
且已脱敏、截断的命令、文件、工具、搜索等摘要，最多保留 100 项并只渲染末尾 20 项；Turn
完成或 Surface 关闭时定稿。该切片不读取原始协议对象、不持久化命令正文或消息 ID，已完成
离线验证，仍待真实飞书验收。

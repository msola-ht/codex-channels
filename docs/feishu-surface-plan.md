...
当前进度（2026-07-26）：已锁定官方 Node SDK `1.71.1`，并完成 Phase 0 的长连接生命周期窄封装、
...
当前私聊命令实现已经完成并可独立审查。Gateway 重启绑定恢复、命令中心、分类卡、状态和主要
选择卡已经通过真实验收；未授权/重复真实事件、新增输入卡和直接命令矩阵仍须补充真实验证。

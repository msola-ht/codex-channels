...
Application 图片输入。后续按真实私聊输入补齐官方 `post` 中的一张图片与可选说明文字，并在同次
Application 提交中保留原始说明文字；多图、一般文件、群聊媒体和下载重试仍不在本切片范围。
真实飞书独立 PNG/JPEG 图片消息主路径已经通过验收，图片说明文字仍待验收；大小超限、无效签名
等失败路径仍由离线测试覆盖。

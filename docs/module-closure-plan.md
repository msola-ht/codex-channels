...
验证记录：命令格式器、三渠道渲染、计划任务动态工具、Bootstrap 启停与 Binding 恢复、Surface 装配及
模块边界共 10 个定向测试文件通过（177 项通过、1 项跳过）；`npm run check`、`npm run lint`、
...
验证记录：指标 Schema、Store、迁移、代理采集、Application/Surface、CLI 与 WebUI 共 14 个定向
测试文件通过（255 项通过、5 项跳过）；审查修复后的数据库专项测试通过（44 项通过、5 项跳过），
指标导出格式测试通过（3 项）；`npm run check`、`npm run lint`、`npm run docs:check`、WebUI 构建、
CLI 专项测试与 `git diff --check` 通过。迁移测试覆盖 v13 私有备份、保留字段事务重建、遗留列与 View
删除、失败回滚，以及重建后继续写入的自增 ID。
...
六个批次均已完成。最后一批已提取纯响应指标观察器、拆分指标 Store 窄端口，并将指标库升级到
Schema v14：停机后创建私有备份，在单一事务中保留仍受支持的请求、额度、账户与子代理数据，删除
价格、成本、旧计时列、派生 View、相关排序字段和直接 API 会话分栏；升级失败回滚原库并保留备份。

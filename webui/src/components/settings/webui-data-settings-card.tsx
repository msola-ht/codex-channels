    ...
    <CardContent className="flex flex-col gap-5 text-sm">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="grid gap-x-8 gap-y-3 md:grid-cols-2">
          ...
          <div className="flex min-w-0 flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between sm:gap-3"><Label htmlFor="webui-port" className="text-muted-foreground">监听端口</Label><div className="flex min-w-0 gap-2"><Input id="webui-port" className="min-w-0 flex-1 sm:w-[130px] sm:flex-none" type="number" min={1} max={65535} value={port} disabled={disabled} onChange={(event) => setPort(event.target.value)} /><Button variant="outline" size="sm" disabled={disabled} onClick={savePort}>保存</Button></div></div>
        </div>

...
export function ThreadsPage() {
  ...
  const { data, loading, error } = useThreads(query)
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-6">
      ...
      {error !== null ? null : loading || data === null ? <PageSkeleton rows={5} /> : (
        <>
          <QuerySummary aggregate={data.aggregate} range={data.range} turns={data.turnCount} />
          <ThreadTable threads={data.threads} query={query} pagination={pagination(data)} />
        </>
      )}
    </div>
  )

...
src/
  ...
  components/  layout（Sidebar）、metrics（指标区块、query-filters 共用筛选栏、query-summary 期间汇总与共享数据表格）、requests（请求明细数据表格）、traffic（转储摘要表格、明细与请求内容/参数对照）、settings（按设置域拆分的卡片与控件）

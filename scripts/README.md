...
- `source-update.mjs` / `source-update.d.mts`：在 `~/.codex-connect/codex-channels` 精确 Git
  ...
  使用实际 CLI 校验候选公开合同、本地审批允许值和 Codex 用户配置；无新提交与 Registry 安装也
  执行同一只读检查。不一致时在停服和切换前失败，不自动迁移审批策略；通过后才原子切换源码，
...
- `codex-public-cli-contract.mjs`：从锁定 CLI 的公开帮助提取 Remote 实际转发参数的存在性、别名、
  参数形状和枚举值，校验用户设置审批值与快照一致；升级时刷新受控快照并按新增、删除、签名变化

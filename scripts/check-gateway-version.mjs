...
const gatewayMetadata = JSON.parse(
  readFileSync(resolve(root, "gateway/src/version.json"), "utf8"),
);

if (packageMetadata.version !== gatewayMetadata.version) {
  throw new Error(
    `Gateway 版本不一致：package.json=${packageMetadata.version}，gateway/src/version.json=${gatewayMetadata.version}`,
  );
}

...
function parseHtmlDocument(html) {
  ...
  while (cursor < cleaned.length) {
    ...
    if (closing < 0) throw new Error("DeepSeek 官方价格页 HTML 标签未闭合");
    const tag = parseTag(cleaned.slice(opening + 1, closing));
    if (tag) handleTag(tag);
    cursor = closing + 1;
  }
  if (table || row || cell) throw new Error("DeepSeek 官方价格页表格未闭合");
  return {
    tables,
  ...
  function handleTag({ name, closing }) {
    ...
    if (name === "table") {
      if (closing) {
        if (!table) throw new Error("DeepSeek 官方价格页表格结构无效");
        tables.push(table);
        table = null;
      } else {
        if (table) throw new Error("DeepSeek 官方价格页不支持嵌套表格");
        table = [];
      }
      return;
    }
    ...
    if (name === "tr") {
      if (closing) {
        if (!row || cell) throw new Error("DeepSeek 官方价格页表格行无效");
        if (row.length > 0) table.push(row);
        row = null;
      } else {
        if (row) throw new Error("DeepSeek 官方价格页表格行嵌套");
        row = [];
      }
      return;
    }
    if (name === "td" || name === "th") {
      if (closing) {
        if (!row || cell === null) throw new Error("DeepSeek 官方价格页单元格无效");
        row.push(normalizeText(cell.join(" ")));
        cell = null;
      } else {
        if (!row || cell !== null) throw new Error("DeepSeek 官方价格页单元格嵌套");
        cell = [];
      }
    }
  }

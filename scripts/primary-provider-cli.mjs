...
export async function runPrimaryProviderCli(
  ...
  if (subcommand === "recover") {
    ...
    if (yes !== "--yes") {
      const confirmed = await prompts.confirm({ message: `恢复 ${id} 模型目录（如有上下文联动事务，将同时恢复关联的 DS、RS、Cline Pass 目录及 Profile）：${action === "keep" ? "保留新目录" : "回滚上一目录"}？请先停止对应服务并核对配置。`, initialValue: false });

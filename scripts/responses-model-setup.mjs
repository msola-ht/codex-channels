...
export async function promptResponsesModels(prompts, defaultModel, previous = []) {
  ...
  for (let index = 0; index < ids.length; index++) {
    ...
    if (keep) {
      ...
      const context = await prompts.text({ message: `${id} 上下文窗口（Token，请按平台说明填写）`, initialValue: String(old?.contextWindow ?? ""), validate: value => Number.isSafeInteger(Number(value)) && Number(value) >= 1024 && Number(value) <= 100_000_000 ? undefined : "请输入 1024-100000000 的整数" });
      ...
      models.push({ id, name: String(name), contextWindow: Number(context), reasoningEfforts, defaultReasoningEffort: defaultReasoning, supportsImages: images });
    }

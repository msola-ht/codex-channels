...
- 提交前确认适用规则仍在上下文中；连续任务内已完整读取且未变化的文件不重复读取。上下文压缩、
  模型切换、长时间中断、规则变化或任务范围扩大后重新读取。
...
- 升级锁定的 Codex CLI 时，只采用 `openai/codex` GitHub Releases 中非 Draft、非 Pre-release
  ...
  保留兼容层。
- 正式升级提案必须运行协议检查、类型检查、Lint、测试、真实合同、构建
...
- `-rc.N` 候选版或 `-fixN` 修复版只允许在准备对应发布 Tag 的提交及其不可变 Tag 中短期存在。
  npm 与 GitHub Release 核验成功后，必须立即通过独立 PR 把 `package.json`、锁文件、`src/version.json` 和
  README 的 `main` 开发基线恢复为无后缀基础版本，同时保留已发布后缀版本的用户安装入口和
  正式版未发布状态。恢复 PR 合并并确认旧版源码更新器接受 `main` 版本之前，不得宣告发行闭环，
  也不得开始下一轮版本发布准备；恢复基础版本不等于正式发布，不得因此创建正式 Tag、移动 npm
  `latest` 或把 GitHub Pre-release 改为正式 Release。已经发布的 Tag 和 npm 包不得移动或覆盖。
...
- 普通提交由 `.githooks/pre-commit` 执行一次完整 `verify:commit`，不提前手动重复运行。
  ...
  `npm run hooks:install`，并手动执行同一完整检查。CI/门禁本身正在修改、用户明确要求或需要独立
...
- 常规文档审查不包含 `.codex/skills/**`；只有用户明确要求安装、更新或审查技能时才单独处理技能目录。

...
describe("metrics export display helpers", () => {
  ...
  it("exports request latency and echoed model independently of turn timing", () => {
    const result = {
      ...
      records: [{ recordedAtMs: 0, weeklyQuota: null, firstContentMs: 12.5, totalDurationMs: 1234.5, tokensPerSecond: 100.125, generationTokensPerSecond: 150.5, upstreamTtftMs: 672,
    ...
    for (const [field, value] of Object.entries({ generationTokensPerSecond: "150.5", tokensPerSecond: "100.125", firstContentMs: "12.5", totalDurationMs: "1234.5", upstreamTtftMs: "672", requestModel: "requested", responseModel: "echoed",
      requestServiceTier: "priority", serviceTier: "default",
    ...
    expect(headerCells).toContain("生成 Token/s");
    expect(headerCells).toContain("端到端 Token/s");
    ...
    expect(markdown).toContain("12.5 ms | 1.23 s | 150.50 | 100.13 | 672 ms | requested | echoed");

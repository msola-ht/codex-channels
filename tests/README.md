...
- Bootstrap 部分启动回滚、启动中停止的单次组件关闭、重连取消与关闭等待。
- 通用 Surface 启停、按账号输出路由与失败隔离；Telegram 格式、通知降噪、长回复折叠与文件回退、
  输出队列顺序与并发关闭等待、生命周期、API 重试及图片输入。
- 飞书官方 SDK 事件长连接的凭据预检、真实握手就绪、启动失败与超时、重连状态、停止竞态、
  SDK 消息字段裁剪、运行状态门控和错误脱敏；文本发送的精确 `chat_id` Payload、有限 HTTP
  超时、SDK 错误脱敏和残缺响应失败关闭；私聊文本 Inbox 的账号/类型/授权筛选、同步有界
  入队、事件去重、旧事件过滤、同 Chat 顺序、跨 Chat 并行、过载重试和有限关闭；所有关键
  `OutputEvent` 的纯文本回退、非关键进度忽略和上游错误详情隐藏；Outbox 的精确账号路由、
  同 Chat 顺序、跨 Chat 并行与关闭等待；已授权文本到 Application 的提交、活动 Turn 追加提示、
  结构化用户错误和未知异常脱敏；单账号 Surface 的长连接启停、关闭排空、输入过载提示和配置通知
  失败关闭；审批拒绝、空用户输入和 MCP elicitation 取消的失败关闭交互端口；当前仍未注册。
...
- TOML、热加载分类、标准环境变量、macOS 系统代理和 Linux GNOME 代理的优先级；无代理时不
  注入空环境变量。
- CLI Doctor 的严格 TOML Schema 校验、共享 App Server 握手与实际版本匹配、敏感错误清洗和只读诊断；项目规则限定当前 Workspace、

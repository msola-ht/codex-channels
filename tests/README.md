...
- 飞书官方 SDK 事件长连接的凭据预检、真实握手就绪、启动失败与超时、重连状态、脱敏生命周期
  日志、停止竞态、SDK 消息字段裁剪、运行状态门控和错误脱敏；文本与 `post + md` 富文本发送的
  精确 `chat_id` Payload、平台原生提及标签中和、有限 HTTP
  超时、SDK 错误脱敏和残缺响应失败关闭；私聊文本 Inbox 的账号/类型/授权筛选、同步有界
  入队、事件去重、旧事件过滤、同 Chat 顺序、跨 Chat 并行、过载重试和有限关闭；卡片动作
  稳定字段裁剪、受限字符串动作值、畸形输入失败关闭和 WebSocket 独立分流；所有关键
  `OutputEvent` 的富文本最终回复、纯文本安全回退、非关键进度忽略和上游错误详情隐藏；Outbox
  的精确账号路由、同 Chat 顺序、跨 Chat 并行、纯文本 UTF-8 与富文本序列化内容的 20,000
  字节上限及每个逻辑结果最多 5 条的安全分片、
  明确截断与关闭等待；已授权文本到 Application
  的提交、活动 Turn 追加提示、命令参数透传、全部平台无关命令结果种类与 Outcome、模型视图、
  非空集合、会话列表条数与预览边界、Diff、Plan、Goal、本地帮助/身份/取消、未知斜杠命令
  失败关闭、输出队列拒绝不重试状态修改、结构化用户错误和
  未知异常脱敏；单账号 Surface 的长连接启停、关闭排空、输入过载提示和配置通知
  失败关闭与安全发送；审批拒绝、空用户输入和 MCP elicitation 取消的失败关闭交互端口；按严格
  配置注册及允许名单热加载。
...
- SQLite 最小绑定恢复、当前版本 Schema 缺失失败关闭、配置热加载与自动重启分类、Setup
  类别与通讯渠道菜单、Telegram Setup、飞书手动输入与扫码注册的消息最小权限、卡片动作回调
...
- 飞书卡片动作字段裁剪、私聊审批卡片、一次性令牌、Actor/Chat/消息/请求绑定、请求原值决定、
  越权与重复动作、超时、关闭竞态和跨客户端失效；用户输入与 MCP elicitation 继续失败关闭。

...
- 飞书官方 SDK 事件长连接的凭据预检、真实握手就绪、启动失败与超时、重连状态、脱敏生命周期
  ...
  结束设置的精确 Payload、递增序列与 UUID、短回复富文本保留、300 ms 增量合并、单卡滚动、
  代码围栏衔接、卡片与回退共用五条预算、明确截断、失败卡片尽力结束、UTF-16 摘要边界、
  Turn/关闭收尾及创建/更新失败后的完整富文本回退；消息资源图片下载的精确
  ...
  `OutputEvent` 的富文本最终回复、启动环境与脱敏 UA、每轮上下文和设置、纯文本安全回退、
  非关键进度忽略和上游错误详情隐藏；Outbox
  的精确账号路由、同 Chat 顺序、跨 Chat 并行、纯文本 UTF-8 与富文本序列化内容的 20,000
  字节上限及每个逻辑结果最多 5 条的安全分片、
  明确截断与关闭等待；同一 Thread 的 active/idle 状态消息创建、重复抑制、顺序更新、更新错误
  分类、失败绑定清理和关闭超时后的迟到结果隔离；已授权文本和 PNG/JPEG 图片到 Application

...
- Bootstrap 部分启动回滚、启动中停止的单次组件关闭、重连取消与关闭等待；内置 Surface 插件
...
- 飞书官方 SDK 事件长连接的凭据预检、真实握手就绪、启动失败与超时、重连状态、脱敏生命周期
  ...
  运行帧忽略、终态静态 CardKit 发送及会话顺序；已授权文本、PNG/JPEG 独立图片和单张图片说明
  文字到 Application 的同次提交、10 MiB 限制、内容签名校验、私有暂存、过期清理、活动 Turn
  追加提示、命令参数透传、
...
- 微信运行时前置协议 Client 的固定 `getupdates/sendmessage` 请求、原始 `message_id` 精度、
  文本与忽略事件裁剪、账号方向、4000 码元出站边界、响应体上限、API/HTTP 错误脱敏、超时和
  取消；版本 1 游标 Store 的账号哈希文件名、严格载荷、原子替换、权限修复、精确删除、损坏与
  符号链接失败关闭；接收监控器的顺序投递、原始 ID 去重、整批成功后游标提交、失败保留旧游标、
  有限瞬时重试、长轮询超时和取消；私聊文本输入 Adapter 的授权、Actor 记录、Application 提交、
  未授权确认与上下文撤销、稳定致命错误、重复启停和有限关闭；内存回复上下文、账号隔离、
  最终文本与必要结束状态、4000 码元代理对安全分片、五片截断、同会话顺序、跨会话并行、发送前
  及分片中途撤权复查、缺少上下文、过载拒绝、关闭清理和日志脱敏，以及三类交互请求立即安全
  拒绝或取消；完整微信 Surface 的稳定身份、输入到最终输出闭环、输入先停再排空输出、重复启停、
  致命故障裁剪、主动配置通知失败关闭和停止后拒绝重启；安全凭据延迟加载与单次缓存、缺失凭据
  失败关闭、精确账号/Actor Policy、显式启用配置、内置插件注册、允许名单热加载和撤权绑定清理。

...
describe("ConversationCommandService", () => {
  ...
  it("covers every registered command through the shared dispatcher", async () => {
    ...
    const service = {
      ...
      previewSessionCleanup: vi.fn(async () => ({ maxTurns: 3, candidates: [], token: null })),
      archiveSessionCleanup: vi.fn(async () => ({ maxTurns: 3, archived: [], failed: [] })),
    ...
    const cases = [
      ...
      ["session-cleanup", "3", "previewSessionCleanup"],
      ["new", "", "newSession"],
      ["archive", "", "archive"],

...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  ...
  可更新消息，启动通知和每轮上下文状态已完成离线实现，私聊 PNG/JPEG 图片和命令审批卡片
  主路径已通过真实应用验收，原生流式、状态消息更新、启动/每轮状态、用户输入与 MCP 卡片仍待验收，
  当前不支持飞书群聊或一般文件。
...
已有企业自建应用；流程只申请机器人发送消息、原生 CardKit 流式卡片、`im.message.receive_v1` 事件和
...
修改事件订阅方式等敏感开发配置。已有应用需要增量开通 `cardkit:card:write` 并发布新版本，
无需重新扫码。当前接收允许名单用户的私聊文本；私聊 PNG/JPEG 图片输入
...
同一 Thread 的 `active → idle` 状态通过飞书消息编辑接口更新原有纯文本状态消息；重复
...
Thread 状态消息更新、用户输入与 MCP form/URL elicitation 卡片已完成离线实现；卡片复用同一
回调、一次性令牌和 Actor/Chat/消息绑定，但这些路径仍待真实验收。

...
扫码或手工配置后，可在飞书私聊发送 `/feishu doctor`。Doctor 以当前 Gateway 已收到的消息、
卡片动作和菜单事件为优先证据，并只读复查应用已开通的租户权限与已发布配置；卡片只显示四项
摘要和当前仍需处理的入口。缺少应用权限时，一次性授权按钮只申请差集；菜单、事件、回调或
待发布版本仍有缺项时，只提供当前应用入口。Gateway 不会自动修改应用能力、事件、回调或菜单，
...
远端快照；只有应用权限仍有缺项时才显示一次性授权按钮，并只申请缺失权限。该动作以一次性令牌
绑定 App、Chat 和 Actor，完成官方应用授权及目标 App 与租户校验后重新执行只读检测；Gateway
不写入或发布应用配置。菜单、事件、回调或待发布版本仍需处理时，卡片只提供当前应用入口，由
App Owner 人工开启菜单、添加 `codexc_home`、确认消息/菜单事件与卡片回调并发布版本。
...
达到预算时在最后一张卡片明确标记截断。官方频控响应只跳过受限的中间更新，保留累计正文和
递增序列供后续自然增量继续，不主动重试；卡片创建或其他内容更新失败时会尽力结束已显示的卡片，

...
- 在 Telegram 中发送文本、最长 5 分钟且最大 20 MiB 的一次性音频、PNG/JPEG 图片和最多
...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；启动通知、短回复、命令结果、
  ...
  20 MiB 和 Codex CLI 支持的格式；飞书独立文件消息当前只接受最多 1,000,000 字节、
  不含二进制控制字符的 UTF-8 文本，在授权后通过官方消息资源 API 下载并以内联文本提交，
  不保存文件副本或暴露本地路径；Office、压缩包、其他二进制文件和群聊仍不支持。
- 微信单账号私聊文本、图片、一次性音频与 UTF-8 文本文件 Surface 已接入：`codexc setup` 会在明确确认连接替换风险后扫码，把
  ...
  Gateway 不保存文件副本，也不要求 Codex 读取工作区外路径。单条微信语音有可信转写时优先作为
  文本输入；否则仅接受 Codex CLI 可直接读取的 MP3/OGG，常见 SILK 当前明确拒绝。Office、压缩包、
...
接收允许用户的私聊文本、一次性语音、UTF-8 文本文件、图文消息与最多 4 张 PNG/JPEG 图片；同一接收批次内的连续图片会在
一秒静默窗口内合并后一次提交，并在独立
...
不显示 Token。微信暂不支持 SILK 解码、实时语音、语音输出、其他二进制文件、视频、群聊或脱离已保存回复上下文的一般主动推送；

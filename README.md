...
Unix WebSocket Upgrade 与原生 `codex --remote` 一样不附加 `User-Agent`、`Origin` 或认证头；Gateway 通过 initialize 的 `clientInfo` 如实标识自身，并显式关闭实验 API 与 attestation capability，不冒充 Codex TUI。未来启用远程 WebSocket 认证时，Authorization 必须由显式安全配置提供，不能隐式复用 Codex 登录凭据。
...
安装或升级后可运行 `codexc doctor` 做一次完整诊断。它会检查 Node 与 Codex CLI 版本、配置文件权限、Telegram 必填项、Workspace、Unix WebSocket 以及 macOS launchd 状态；Token 只检查是否存在，不会显示内容。诊断发现关键问题时以非零状态退出。
...
`service install` 生成并启动 App Server 与 Gateway 两个独立的用户级 launchd 服务；重复执行时会先卸载旧实例，再加载新 plist。从旧版升级时，安装流程会停止并删除 `com.msola.*` Job 和 plist，再切换到 `com.hegenai.*`，保留 `~/.codex-connect` 中的配置与运行数据。Linux 当前没有系统服务安装器，可使用 `codexc start` 前台运行。用户配置目录解析本身不依赖 macOS，但 Windows 尚未适配当前 Unix WebSocket Transport，暂不属于可运行平台。
...
GitHub Actions 会在 push、Pull Request 和手动触发时，分别使用 macOS 与 Linux、Node.js 22.13.0 执行依赖锁定安装、类型和版本检查、单元测试，以及真实 tarball 隔离安装冒烟；macOS 任务还会检查 launchd 模板。依赖本机 `codex-cli 0.145.0` 的协议检查和真实 App Server 冒烟测试仍按上述命令手动执行。

## npm 发布

`publish.yml` 使用 npm Trusted Publishing：推送与 `package.json` 一致的 `v*` Tag 后，GitHub Actions 会重复执行检查和 tarball 冒烟，再通过 OIDC 发布，不保存长期 npm Token。首次启用时，在 npm 的 `@hegenai/codexc` 包设置中添加 GitHub Actions Trusted Publisher：仓库填写 `msola-ht/codex-channels`，工作流填写 `publish.yml`，Allowed actions 选择 `npm publish`。
...
工作流会拒绝 Tag 与包版本不一致的发布。正式打 Tag 前可运行 `npm run test:package` 验证实际 npm tarball；脚本会在隔离目录安装包，并执行 `codexc --version` 与 `codex-connect --help`。

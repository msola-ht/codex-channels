...
- 查看 Codex 流式回复、格式化最终回复、操作过程、计划、Diff、Goal、用量和额度；长文本自动折叠，超长代码以预览加完整文件发送；`/status` 和每轮结束状态卡显示当前 Goal 状态及 Thread 上下文压缩总次数。
...
握手和系统服务状态，但不会显示 Token。项目不读取或迁移旧 `.env` 配置。

...
`doctor` 会检查 Node、Codex CLI、配置权限与兼容性、Telegram 必填项、Workspace、App Server
握手和系统服务状态，但不会显示 Token。升级后如检测到明确支持修复的旧配置，可由用户主动执行
`codexc doctor --fix` 原子改写配置；该命令不会静默迁移未知配置，也不会覆盖已经存在的当前配置值。
...
codexc doctor [--fix]            # 诊断；显式修复已知旧配置

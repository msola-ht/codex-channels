...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  飞书富文本，模型增量超过短回复等待窗口后使用原生 CardKit 流式卡片，错误与操作性提示
  ...
  `/start`、`/help` 与机器人自定义菜单可打开只读命令中心卡片；`/feishu doctor` 可在 Actor
  明确确认后完成飞书官方应用授权、重新执行只读检测并给出人工配置指引；Gateway 不自动修改
  菜单、事件或回调，也不发布应用版本。Doctor 会分别检查菜单节点和启用开关；节点存在但未启用时
  显示“已定义但未启用”。当前不支持飞书群聊或一般文件。
...
扫码或手工配置后，可在飞书私聊发送 `/feishu doctor`。Doctor 通过一次性卡片完成飞书官方
应用授权，然后只读复查长连接消息事件、卡片回调和事件类型机器人菜单 `codexc_home`。Gateway
不会自动修改应用能力、事件、回调或菜单，也不会创建或发布应用版本；卡片会提供当前应用入口，
提示 App Owner 人工开启自定义菜单、添加 Event Key 为 `codexc_home` 的事件类型菜单项、确认
长连接事件与回调并发布版本。新扫码应用只声明对话、流式卡片、只读检测及所需事件/回调，不申请
应用配置写权限。已有应用需要增量开通 `cardkit:card:write` 并由 Owner 发布新版本，无需重新创建应用。
当前接收允许名单用户的私聊文本；私聊 PNG/JPEG 图片输入
已完成代码与离线测试，部署后仍需真实应用验收。审批请求不会通过飞书静默批准高权限操作。
...
卡片回调、机器人菜单事件和当前 Actor OAuth 状态；`doctor` 合并必要应用能力、事件/回调配置与精确申请入口。
Doctor 还会读取当前已发布版本与应用开发配置；存在缺项且没有其他待处理版本时，卡片提供
“授权并查看配置指引”。该动作以一次性令牌绑定 App、Chat 和 Actor，完成官方应用授权及
目标 App 与租户校验后重新执行只读检测；Gateway 不写入或发布应用配置。卡片提供当前应用入口，
由 App Owner 人工开启菜单、添加 `codexc_home`、确认消息/菜单事件与卡片回调并发布版本。
已发布菜单只有在 `bot_menu_enable=true` 时才显示为“已启用”，节点存在但开关关闭时显示
“已定义但未启用”。命令在飞书授权和有界输入队列之后进入同一个
`ConversationCommandService`；未知或畸形斜杠命令会明确拒绝，不会作为普通消息提交给
Codex。短回复和命令结果使用飞书 `post + md` 富文本；持续产生的模型正文使用 CardKit 2.0
原生流式卡片，错误、过载和操作性提示保留纯文本。
`/start` 和 `/help` 会发送命令中心卡片；机器人自定义菜单的 `codexc_home` 事件复用同一入口，
当前只提供状态、会话列表、Workspace、模型、账户用量和完整文本帮助六个只读动作。菜单事件
本身不携带 Chat ID，因此只有当前 Actor 恰好匹配一个已授权私聊绑定时才会响应；事件和卡片
令牌均为有界内存状态，重启后清空。卡片动作继续调用同一个 `ConversationCommandService`，
不会把按钮伪装成用户文本，也不会从卡片直接触发写操作。
Gateway 长连接就绪后，会向已有授权绑定发送富文本启动通知；每轮结束会使用
...
最多发送 5 条，超出时明确标记截断，不依赖平台接受无界消息。会话列表最多展示 20 条，长预览会规范空白并截断，
可通过 `/sessions <搜索词>` 或 `/archived <搜索词>` 缩小范围。
同一 Thread 的 `active → idle` 状态通过一条轻量交互卡片原地更新；蓝色表示运行中，绿色表示
空闲。重复
`active` 不产生额外更新，更新失败不自动重试，也不阻塞后续输出。
消息创建失败不会自动改发另一种格式，以免非幂等重发产生重复消息。
流式增量以 300 ms 合并并继续经过同一 Chat 的有界顺序队列；单卡内容采用保守的 5,000
Unicode 字符上限，并在代码围栏跨卡时闭合和重开。流式卡片与失败回退富文本合计最多 5 条；
达到预算时在最后一张卡片明确标记截断。卡片创建或内容更新失败时会尽力结束已显示的卡片，
Turn 完成后再用剩余预算回退完整富文本；已显示完整内容后的结束设置失败不重复发送正文。
持续回复在飞书客户端可见原生流式更新的主路径已通过真实应用验收；真实限流、失败回退和
超长内容滚动仍待验收。
私聊图片使用同一授权、去重和顺序队列；只接受 PNG/JPEG，最大 10 MiB，下载后写入
`~/.codex-connect` 下的私有临时目录并定期清理；私聊 PNG/JPEG 真实消息主路径已通过验收，
一般文件未实现。飞书命令审批卡片及 `card.action.trigger` 动作回调已通过真实应用验收；
Thread 状态卡片更新已通过真实应用验收；用户输入与 MCP form/URL elicitation 卡片已完成离线

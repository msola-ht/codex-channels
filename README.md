...
- 微信单账号私聊文本、图片、语音转写与 UTF-8 文本文件 Surface 已接入：`codexc setup` 会在明确确认连接替换风险后扫码，把
  ...
  上下文接收 Gateway 重启上线通知。Gateway 启停还会调用锁定微信合同的
  `notifystart` / `notifystop` 完成服务端在线状态对账；对账或聊天通知失败都不会停止长轮询，
  聊天通知失败时保留最近回复上下文，等待后续入站消息自然刷新。微信命令结果会把结构化标题和
  字段转换为紧凑 Markdown 标题与项目符号，生命周期结构化字段同样使用项目符号，不再为
  每个字段额外插入空行。微信最终回复会按锁定上游的
  兼容集合保留标题、粗体、删除线、
  引用、列表、表格、分隔线和代码格式；中文斜体会降级为普通文字，H5/H6 会降级为粗体标题，
  普通 HTTP(S) Markdown 链接会保留并交给微信客户端渲染；Markdown 图片标记会移除且不会触发网络下载，
  图片仍只由既有的原生图片通道发送。
  单行 Markdown 代码块会压缩为行内代码，避免客户端生成占空间的 `TEXT / 复制` 区域；
  多行代码块仍保留复制能力，未闭合代码围栏降级为普通文本。
...
- Telegram、飞书和微信共用一套生命周期信息模型：Gateway 上线时按相同顺序通知当前系统、版本、
  ...
  错误、上下文、缓存、模型、压缩、周限、Goal、Git 分支和耗时。Telegram 与微信使用紧凑项目符号，
  飞书使用卡片列表；各平台只保留自己的消息外观。
...
`[内部路径]`，普通项目路径保持可核对。Telegram 和微信会把同一 Turn 内成功完成的 MCP 工具、
动态工具和网页搜索延迟到最终回复前统一发送；飞书的网页搜索完成后立即发送，MCP 与动态工具
仍按 Turn 聚合。单项保留详情，多项只发送一次分类计数汇总，失败和拒绝仍即时显示。飞书在两种模式下都把耗时
...
开启后三个渠道先发送一次完整计划。飞书在同一 Turn 内固定使用这一张计划卡，后续状态全部
原地更新；Telegram 和微信保留首次完整计划消息，并在每个步骤首次完成时单独发送紧凑进度。
该能力只消费官方 `turn/plan/updated`，不从模型正文
推断步骤或拆分普通回复，也不改变 `/plan` 协作模式。进度发送时间以 Codex 实际上报步骤状态的
时间为准；如果多个步骤只在同一次官方通知中完成，Gateway 无法补发这些步骤过去的完成时点。
...
最终回复默认把常用 Markdown 安全转换为兼容性更好的 Telegram HTML；HTTP(S) 链接会转换为
可点击链接，Markdown 表格会降级为紧凑的粗体表头与项目符号行。支持 Rich Messages
...
- 快捷命令：`/h` 等价于 `/help`，`/work` 等价于 `/workspace`，`/r` 等价于 `/resume`

上述快捷命令由 Telegram、飞书和微信共同支持，但不重复加入平台命令菜单；帮助内容按会话、
运行与项目、模型与能力分组显示，完整命令名仍是文档和菜单中的规范入口。

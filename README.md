...
`/skills` 只显示用户直接安装且已启用的个人 Skill，不重复列出系统、项目或插件附带 Skill；`/plugins` 只查询本机已安装插件，不加载远端插件市场目录。
`/fast off` 会在下一次 Turn 显式选择 Standard 层级，不会重新继承全局 Fast 默认值；Turn 成功启动后，该设置由 App Server Thread 保存，Gateway 重启或重建服务后会随 Thread 恢复。

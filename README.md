...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  ...
  明确确认后增量配置菜单、事件和回调并提交版本。菜单链路与一键配置已完成离线实现、待真实
  应用验收。当前不支持飞书群聊或一般文件。
...
扫码或手工配置后，可在飞书私聊发送 `/feishu doctor`，通过一次性卡片明确确认后增量配置
长连接消息事件、卡片回调和事件类型机器人菜单 `codexc_home`，并提交应用版本。新扫码应用会
预先申请 `application:application:self_manage` 与 `application:application:patch`；已有应用缺少
应用管理权限时，一键流程会先显示飞书官方配置授权卡片。流程保留其他机器人菜单，不删除权限、
事件或回调；检测到已有未完成版本时失败关闭，避免自动发布无关改动。企业启用应用审核时，
管理员仍需在飞书完成审核。已有应用需要增量开通 `cardkit:card:write` 并发布新版本，
无需重新创建应用。当前接收允许名单用户的私聊文本；私聊 PNG/JPEG 图片输入
...
“确认并一键配置”。该动作以一次性令牌绑定 App、Chat 和 Actor，并先完成官方应用配置授权及
目标 App 与租户校验，再重新读取最新配置，保留已有菜单并增量添加 `codexc_home`、消息事件及
卡片回调；首次检测因
权限不足而失败时也可从同一卡片进入该流程。检测到其他未完成版本时不会自动
发布；提交成功不代表企业管理员已经审核通过。命令在飞书授权和有界输入队列之后进入同一个

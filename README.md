...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  飞书富文本，错误与操作性提示保留纯文本；私聊 PNG/JPEG 图片和审批卡片已完成离线实现，
  仍待真实应用验收，当前不支持飞书群聊或一般文件。
...
`/whoami` 和 `/cancel`。命令在飞书授权和有界输入队列之后进入同一个
...
`~/.codex-connect` 下的私有临时目录并定期清理。该能力尚待真实飞书消息验收，一般文件未实现。
飞书审批卡片已完成离线实现，但新增 `card.action.trigger` 回调必须通过扫码更新已有应用后才能
进行真实验证；用户输入与 MCP form/URL elicitation 卡片也已完成离线实现，复用同一回调、

...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  飞书富文本，错误与操作性提示保留纯文本；同一 Turn 的运行中与空闲状态已实现合并到一条
  可更新消息，启动通知和每轮上下文状态已完成离线实现，私聊 PNG/JPEG 图片和命令审批卡片
  主路径已通过真实应用验收，状态消息更新、启动/每轮状态、用户输入与 MCP 卡片仍待验收，
  当前不支持飞书群聊或一般文件。
...
已有企业自建应用；流程只申请机器人发送消息、`im.message.receive_v1` 事件和
...
修改事件订阅方式等敏感开发配置。当前接收允许名单用户的私聊文本；私聊 PNG/JPEG 图片输入

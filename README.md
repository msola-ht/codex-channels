...
- 微信单账号私聊文本与图片 Surface 已接入：`codexc setup` 会在明确确认连接替换风险后扫码，把
  ...
  加密上传并作为微信图片气泡发送，普通 `imageView` 不会自动外发。语音、文件、视频等其他媒体
  暂不支持。每个 Turn
  ...
  退避、Token 失效暂停或停止状态，以及最近成功时间、连续失败次数和预计恢复时间。
...
接收允许用户的私聊文本、图文消息与最多 4 张 PNG/JPEG 图片；同一接收批次内的连续图片会在
一秒静默窗口内合并后一次提交，并在独立
...
不显示 Token。微信暂不支持图片以外的其他媒体、群聊、一般主动推送或审批；当前图片

...
- 可选启用飞书企业自建应用，通过已授权用户的私聊文本提交 Turn；最终回复和命令结果使用
  飞书富文本，错误与操作性提示保留纯文本；私聊 PNG/JPEG 图片和命令审批卡片主路径已通过
  真实应用验收，用户输入与 MCP 卡片仍待验收，当前不支持飞书群聊或一般文件。
...
用户输入与 MCP form/URL elicitation 卡片已完成离线实现，复用同一回调、一次性令牌和
Actor/Chat/消息绑定，但仍待真实动作验收。

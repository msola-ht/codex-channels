...
- `sqlite-request-metrics-schema.ts`：集中保存当前 Schema v19 建库 SQL、存储列定义、版本错误和
...
- `sqlite-request-metrics-store.ts`：把脱敏后的 Provider、模型、状态、HTTP/传输格式、Usage、
  ...
  渠道卡片和 WebUI 页面展示两个派生输出速率，都不新增存储列。`generationTokensPerSecond`（生成速率）为单请求输出
  Token 除以解码窗口（总耗时减去首内容延迟），Turn/Thread 汇总按「合计输出 ÷ 合计解码窗口」合并计算；
  `tokensPerSecond`（端到端有效速率）为输出 Token 除以请求总耗时，汇总同样按「合计输出 ÷ 合计总耗时」合并计算。
  两者都要求该记录输出 Token 与对应时间窗同时大于零，缺采样的记录整条退出而不是按零参与，也不先把逐请求速率
  平均再汇总：逐请求速率会被极短解码窗口的单条记录放大，分子分母取自同一批记录后相除才不会被短请求等权拉偏。
  两者都不从输出中减去推理 Token，压缩请求沿用原统计范围；生成速率不是含首字等待的产出率，端到端速率不是纯生成
  速度或会话墙钟吞吐量（不含请求之间的编排、工具与空闲时间）。单请求首内容不合成为轮次指标，完成卡片的官方 Turn 总耗时
  不来自本指标库。内部读取限制为每次
...
HTTP API 复用相同查询，不向本模块写入状态。只接受当前 Schema v19，不提供历史迁移；可在停止 Gateway 后使用 `codexc metrics reset` 归档并重建旧库。

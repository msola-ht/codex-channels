...
`display.show_operation_updates` 是 Telegram 与飞书共用的操作过程显示开关，默认开启。关闭时只
抑制 `operation.updated` 的平台输出，不影响审批、错误、最终回复和 Turn 完成事件；变化需要重启
Gateway，不需要重装或重启共享 App Server。

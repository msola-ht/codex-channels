...
- `reload-classifier.ts`：比较两份已验证配置及 Bootstrap 检测的微信凭据变化，完整列出变化，并按热加载、Gateway 重启或服务重装
  的最高要求分类。
...
`experimental.plugin_api` 控制当前锁定 Codex 0.156.1 的开发中 Plugin 调试入口，默认关闭；未显式开启时
`/plugin` 的列表与调用在 Application 边界失败关闭，不发送 `plugin/installed` 或 mention
请求。变化需要重启 Gateway，不需要重启 App Server；Doctor 始终显示开关状态与风险提示。

`scheduled_tasks.enabled` 控制 Gateway 计划任务执行层，默认关闭。开启时才创建独立私有
...
0 只关闭渠道会话自动解除；无任何绑定时的全局空闲释放仍会停止未被租约占用的 App Server。

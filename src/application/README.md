...
- `conversation-service.ts`：新建、恢复、切换、归档和查询 Thread，提交、steer 或将纯文本
  ...
  到当前授权 Workspace；扩展查询通过 `ConversationQueryPort` 组合窄端口，Skill 与 MCP 已
  使用稳定结果，Plugin 与权限仍按收敛计划逐项迁移。
...
扩展查询也保持平台无关：Skill 只返回当前用户或 Workspace 直接安装且已启用的项，排除系统和插件缓存内容；MCP 只返回展示所需的稳定摘要，并按当前 Thread 读取项目级配置；Plugin 只读取已安装项，不触发远端市场目录刷新。

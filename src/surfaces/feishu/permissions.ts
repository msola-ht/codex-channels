...
export function renderFeishuDoctor(
  ...
  return [
    ...
    `机器人菜单事件：${status.menuEventObserved ? "通过" : "尚未验证"}`,
    `当前用户 OAuth：${userAuthorizationLabels[userAuthorization]}`,
    "",
    "当前 Surface 对话必需能力：",
    ...
    `事件订阅：确认已添加 ${requiredMessageEvent} 和 ${requiredMenuEvent}，并选择长连接。`,
    `机器人菜单：添加事件类型菜单项，并将事件 Key 设为 codexc_home。`,

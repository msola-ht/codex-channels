...
- 普通消息发送使用 `im.v1.message.create` 的 `chat_id + text/post/interactive` 窄能力；
  ...
  `im.v1.message.patch`；Thread 状态卡只由 `active` 创建，`idle` 只把已有卡更新为
  “处理结束 · 结果见下方消息”，
...
- 显式回复消息时使用 `parent_id` 按需读取一条被引用消息，需要应用权限
  ...
  已有应用由 Owner 通过 `/fs doctor` 增量开通。飞书“复制消息链接”的 token 没有公开反查接口；
...
按同一 Chat 顺序把蓝色“运行中”更新为绿色“处理结束 · 结果见下方消息”；重复 `active` 被忽略，

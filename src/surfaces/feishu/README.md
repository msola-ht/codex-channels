...
且不阻塞后续输出。蓝色运行中卡片可以原地更新为绿色空闲卡片；通用消息更新尚未实现。超过五张卡片可靠
...
`/vision <要求>` 在当前 Actor 与私聊内预设下一批图片的识别要求；也可使用
`/vision begin <要求>` 逐条收集最多四张图片，再用 `/vision done` 一次提交，`/vision cancel` 取消；

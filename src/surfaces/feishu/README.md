...
未知 Thread 状态不会原样显示。`turn.completed` 使用标题、列表和合并后的模型设置行，把事件
提供的官方 Turn 对话耗时单独放在分隔线后的底栏，并只展开最近 Turn 上下文、缓存命中率、模型
...
操作耗时单独放在分隔线后的底栏。`hidden` 不发送命令、文件、工具或搜索操作终态卡片；审批、
错误、最终回复和 Turn 完成统计保持原有行为。

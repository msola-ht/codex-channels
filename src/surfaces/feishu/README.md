...
纯文字富文本、代码块与 Markdown 富文本、独立图片、最多四张图片说明文字、一次性音频与独立 UTF-8
文本文件 Inbox、输出渲染、
...
- 输出队列关闭或 Conversation 取消时，Outbox 会把 `AbortSignal` 传递到文本、文件、图片、状态卡、
  审批卡和流式卡片的发送、更新与回退路径；尚未投递的 reasoning 卡会在 Turn 进入命令或工具执行后失效，
  不会覆盖“运行命令”状态。取消语义只保证 Gateway 停止等待并阻止后续展示；当前锁定的飞书 Node SDK
  生成接口没有暴露 Axios `AbortSignal` 配置，因此已经发出的底层 HTTP 请求仍由 SDK 自身完成或等待其
  超时，不应将该能力理解为强制中断平台请求。
...
分类入口打开第二张卡片，按“会话查询”“会话操作”“能力与集成”“当前内容”“飞书”展示会话与集成动作，
不复用完整文本帮助。会话切换、会话列表、已归档、模型、思考等级、Fast 和工作区会打开
最多 18 项的选择卡；活动会话选择复用 `resume`，归档会话选择复用 `unarchive`，Fast 首次点击
只读取模型状态，明确选择开关后才执行修改；选择支持多个思考等级的模型后继续打开思考等级选择卡。
会话与归档搜索、重命名、下一 Turn 追加、Review
...
Queue ID、有界页码和分组编号，条目标签只显示 Application 提供的安全预览，不保存 Queue 镜像。
计划任务入口读取共享 `/schedule` 结果，支持前台 Agent 通过 `schedule_task` 工具返回的创建预览，并提供任务分页、多种创建表单（间隔、一次性、每月、每天、工作日、每周）、任务运行记录、立即运行、
暂停/恢复、重命名和删除确认；从聊天直接输入创建或删除命令时也返回原生确认/取消卡片。创建与删除
仍使用 Application 生成的五分钟一次性令牌，按钮不绕过确认。创建与删除预览使用 CardKit 2.0 分组显示任务与执行配置；
确认或取消动作被接受后，原卡片会原地更新为绿色或灰色的无按钮终态，实际执行结果继续使用共享 Application 输出。
项目规则卡只提供 `init` 和 `check`。每张卡片的
令牌除绑定消息、Chat、Actor 和访问策略外，还绑定该卡实际渲染的精确动作与参数；未展示值
失败关闭，选择卡和输入卡接受一次后立即失效；Queue 的启动与确认删除也会消费当前令牌；直接新建、停止、归档、固定、取消固定、压缩或
分叉会话也会消费
...
`post` 富文本、`code_block` 代码块与 `md` Markdown 富文本、独立图片，以及包含最多四张图片和
可选说明文字的 `post`。超链接同时保留可见文字和目标 URL，`code_block`/`md` 抽取原始文本内容；
其余未知元素或畸形结构失败关闭并通过 Surface 明确提示用户，不能丢弃部分内容后提交。SDK 回调只做
...
未知 Thread 状态不会原样显示。`turn.completed` 使用共享标题、字段顺序和合并后的模型设置行，按 Token 与性能分组展开
最近模型请求的上下文、Token 明细、模型设置、压缩次数、周限和 Goal，不查询第二状态源。
...
过程增量和 Thread 状态通过同一卡片流式或原地更新降噪；运行中的命令和工具会先追加进行中消息，
完成后再按顺序追加终态消息。最终回复、
...
会话、命令前缀或精确网络规则，重复、畸形、越权、过期和关闭后的动作均不会升级权限。
审批卡和处理结果卡直接展示短详情；长详情默认展示有界预览，并允许展开核对原始命令、权限与
持久规则。预览和折叠只影响飞书呈现，不裁剪协议决定或改变 App Server 返回值。用户输入

...
Codex 私有目录的绝对路径替换为 `[内部路径]`，并把操作耗时单独放在分隔线后的底栏。成功的 MCP、动态工具和网页搜索按 Turn 延迟到最终回复前
聚合，单项保留详情，多项仅发送一次分类计数；失败与拒绝仍即时发送。`hidden` 不发送命令、
文件、工具或搜索操作终态卡片；审批、错误、最终回复和 Turn 完成统计保持原有行为。
`display.plan_updates = true` 时，官方自动计划先使用一张卡片显示完整步骤；每个步骤首次
完成时再发送一张紧凑进度卡；完整计划卡会在原消息中同步更新，不另发进行中快照，默认关闭。

...
这些路径均已通过离线测试，纯文字富文本输入、最多四图加说明文字的单 Turn 输入、原生流式主路径
与生成图片回传另已通过真实应用验收。审批能力已完成
私聊审批卡片的离线主路径，命令审批的一次批准及当前 Gateway 长连接动作接收已通过真实验收；
私聊 PNG/JPEG 真实消息也已通过。用户输入卡及 MCP 工具审批的一次批准已通过真实验收；
MCP form 与 URL elicitation 已完成离线实现，继续等待真实卡片动作验收。
...
并通过 Outbox 原生回复当前输入；Adapter 不自行重复发送。后续回复由 Core 输出驱动；追加到
...
已完成真实应用验收。
断线恢复、代理、未授权/重复事件与 MCP form/URL 卡片动作仍未完成真实验收；原生用户输入卡和
MCP 工具审批的一次批准已通过真实验收。剩余私聊验收以
[`通讯渠道验收矩阵`](../../../docs/channel-acceptance-matrix.md)为唯一状态入口；一级 `surfaces`

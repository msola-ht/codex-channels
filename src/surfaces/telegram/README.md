...
- `lifecycle.ts`：Bot 命令注册、Long Polling、连续且属于同一原生媒体组的并发准备、包含系统、会话与 Git
  ...
  `/stop` 和审批回调在已拉取的更新中绕过普通聊天积压与接纳失败，仍进入现有授权与一次性交互处理链；确认位置保留失败边界。
  未授权提示通过有并发上限和取消期限的后台任务发送，饱和时省略提示。
  普通更新在同一聊天内保持顺序，不同聊天独立处理。生产注入独立加密日志，可靠接纳后推进
  Long Polling offset，正常配额内不因慢输入占住 100 条确认窗口；独立使用且未注入日志时，只推进连续处理完成的位置并保留当前进程去重。
  关闭时中止该代输入，最多等待轮询、更新和启动通知
  5 秒，未开始的旧输入不再执行；超时记录未完成数量，不记录消息正文。
  Telegram 每次最多返回 100 条最早未确认更新；当前窗口被慢输入占满时，窗口外的新控制命令仍须等待。
  未注入日志的独立模式不持久化未确认正文；进程在业务提交成功、平台确认之前退出时可能重放，不能保证恰好一次。

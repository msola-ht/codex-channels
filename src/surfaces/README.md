...
工作区、Session 名称和 Session ID，并把本次运行、当前会话累计和账户状态依次分区，按 Token、费用、性能分组；按 Turn
...
换算对照；账户状态在正式模式保留精简剩余额度，调试模式才展开额度中心周期明细。子代理完成卡片始终展示从启动到首次官方终态的墙钟耗时，并复用同一价格、Token 和均价格式，
在值可靠时展示思考等级与线程聚合的

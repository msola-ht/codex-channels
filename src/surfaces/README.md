...
不把请求累计输入误写成上下文占用；聚合中的上下文压缩摘要单列请求数与 Token，不显示总耗时、
首段回复延迟、生成速度、本地价格或费用。信息类聊天指令（`/status`、`/usage`、
...
工作区、Session 名称和 Session ID，并把本次运行、当前会话累计和账户状态依次分区；按 Turn
...
`elapsed-duration.ts` 只把已确认的操作、推理状态等毫秒值或账户用量秒数格式化为三个 Surface

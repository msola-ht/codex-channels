...
聚合耗时、首段回复延迟、纯生成速度、本地价格或费用；完成卡片的运行、任务与会话摘要展示指标库派生的
输出 `Token/s`（合计输出 Token 含推理，除以合计请求耗时，含首字等待），未采样时整行省略、不渲染占位文本；
`/metrics` 的最近运行聚合与当前会话累计使用同一读数，直接复用查询结果，不用会话墙钟耗时重新计算。

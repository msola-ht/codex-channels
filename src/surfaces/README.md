...
综合速度和首段回复延迟附带有效样本覆盖率，不把请求累计输入误写成上下文占用；聚合中的上下文
压缩摘要单列请求数与 Token，不显示本地价格或费用。信息类聊天指令（`/status`、`/usage`、
...
工作区、Session 名称和 Session ID，并把本次运行、当前会话累计和账户状态依次分区，按 Token 与性能分组；按 Turn
...
成功”，底层异常记录仍完整保留。完成卡片正式模式保留 Token 总计，调试模式才展示模型请求
聚合耗时并展开 Token 子项；账户状态在正式模式保留精简剩余额度，调试模式才展开额度中心周期明细。
子代理完成卡片始终展示从启动到首次官方终态的墙钟耗时，
在值可靠时展示思考等级与线程聚合的
综合输出速度（不含推理，并标明计时覆盖率）：正式模式保留总计，调试模式才展开缓存与推理 Token
与缓存命中率；指标读取失败时只显示“统计暂不可用”。

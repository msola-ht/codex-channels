...
Telegram 的独立操作消息各自作为一项输出，整批操作不共用一个截止时间。
排队或执行达到 5 秒时记录告警，包含队列剩余数量和耗时；调试日志区分任务执行完成与失败，
`runOrdered` 调用者收到的拒绝也计入失败。队列任务完成不代表平台已确认消息，平台确认由渠道 API 日志记录。
...
聚合耗时、首段回复延迟、纯生成速度、本地价格或费用；完成卡片的运行、任务与会话摘要在每处速度读数上展示指标库派生的
生成速率 `Token/s`（合计输出除以合计解码窗口）与含首字等待的端到端 `Token/s`（合计输出除以合计请求耗时），
两个读数各自在未采样时省略、不渲染占位文本；`/metrics` 的最近运行聚合与当前会话累计只展示生成速率。
两者都直接复用查询结果，不用会话墙钟耗时重新计算。信息类聊天指令（`/status`、`/usage`、
...
账户状态在正式模式保留精简剩余额度，调试模式才展开额度中心周期明细。子代理完成卡片在值可靠时

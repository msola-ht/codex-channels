...
- `updates-monitor.ts`：组合协议 Client 与游标 Store；单批消息先按原始消息 ID 去重，只并发准备
  ...
  只重新处理失败图片；仅在整批处理成功后提交游标。网络、HTTP 429 与 5xx 瞬时故障先按 2 秒重试，连续 3 次失败后
...
- `conversation-adapter.ts`：复用 Application 的 `ConversationCommandService` 和完整共享命令
  目录，并保留微信本地 `/start`、`/help`、`/whoami`、`/wx doctor`；共享解析器把
  `/h`、`/work`、`/r` 显式规范化为
  `/help`、`/workspace`、`/resume`；`/vision <要求>` 预设下一批图片的识别要求，
  `/vision <2–4> <要求>` 声明多图数量并在收齐后自动提交；兼容的 `/vision begin <要求>`、
  `/vision done` 保留给数量未知的收集，`/vision cancel` 取消。将说明文字和全部成功下载的图片一次

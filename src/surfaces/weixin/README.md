...
- `outbox.ts`：只处理匹配账号且复用共享生命周期字段的 Turn 开始确认、原生输入状态、CLI/TUI
  ...
  思考状态只显示“思考中…”，微信无流式更新，因此每段在首次收到思考事件时立即发送一次，不等待思考结束，
  不发送“思考完成”，也不展示推理摘要或思维链内容；`display.reasoning = false` 时不显示。
  队列关闭或 Conversation 取消时，文本、图片、文件和完成回退会接收取消信号；已接受的关闭收尾仍按

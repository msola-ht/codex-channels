...
- `credential-client.ts`：首次协议调用时从安全存储读取并缓存当前进程的凭据 Client；缺失或
  损坏凭据失败关闭，不把 Token 提升到 Bootstrap 配置或日志。提供进程内凭据变更检查，
  配置重载时只返回未变化、已变化或读取失败，供 Bootstrap 重启同账号的新连接。
  读取失败记录微信故障，不阻止其他渠道启动和重载；启动时不可读的凭据恢复可读后要求重建连接。
- `factory.ts`：接收 Bootstrap 已决策的精确凭据、游标、回复上下文和上传目录，组合微信固定的
  协议 Client、存储与媒体实现，并把同一个代理感知 Fetch 注入 JSON API、CDN 上传和下载；
  一级 Surface 出口只提供组合入口及 Bootstrap 凭据变更检查所需的窄接口。
...
- `quoted-reference.ts`：授权后组合平台正文、当前进程缓存和摘要；依据官方局部引用位置与可选
  ...
  引用正文或不可用提示优先于摘要，避免摘要占满共享引用长度预算。
...
- `updates-monitor.ts`：组合协议 Client 与游标 Store。生产注入批次上下文准备与可靠接纳入口，整批加密落盘后保存游标，普通接纳失败后仍尝试同批控制消息但不推进游标，后台按会话处理与下一批拉取分离；独立使用时保留整批处理后提交游标的合同。
  网络、HTTP 429 与 5xx 瞬时故障先按 2 秒重试，连续 3 次失败后进入 30 秒退避；官方 `-14` 暂停该账号一小时。
  接纳或检查点失败不推进游标，日志负责跨重启有限去重；不确定的业务写操作不自动重发。
...
- `input-adapter.ts`：拥有单账号监控器生命周期；按微信账号和私聊 Actor 构造目标，授权后记录
  Actor；持久模式在批次接纳前选定并加密保存最近回复上下文，后台与恢复只读取，不用日志顺序改写。再把文本、UTF-8 文本文件、图文或最多 4 张图片交给目录内会话 Adapter。协议边界
  ...
  精确命中时补充原文，每条只缓存前 8000 个 Unicode 字符并单独记录截断状态，展示省略号不参与局部引用匹配。
  机器人消息、重启后或淘汰后的未命中引用附带正文不可用提示，消息正文不持久化。
...
- `outbox.ts`：只主动发送匹配账号的 Turn 开始确认、最终正文、完成/停止/失败统计、会话空闲解除
  ...
  验证可用的出站原生回复字段，普通 Turn 继续发送普通消息。
  同一已拉取批次内的紧急停止与普通输入分开执行，普通输入保持顺序，全部处理完成才提交游标。
  输入取消后停止后续提交，较旧输入不会覆盖被紧急指令更新的回复上下文。
  文件投递结果未知或限流时不触发文本回退。
  队列关闭或 Conversation 取消时，文本、图片、文件和完成回退会接收取消信号；已接受的普通输出最多按既有顺序排空 5 秒，交互立即取消。
  截止后中止在途发送，停止后续分片和回退，未开始的任务会被丢弃。微信协议客户端支持在 Fetch 层中止请求，与平台无关的状态展示
  不会因取消继续发送。
